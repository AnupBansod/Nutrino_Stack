...
/* Get properties connection callback */
static bool static_PVirtualTagGetProperties(
            tContext* pContext,
            void* pObject,
            uint8_t* pPropertyArray );

/* Check properties connection callback */
static bool static_PVirtualTagCheckProperties(
            tContext* pContext,
            void* pObject,
            ...
            uint8_t nPropertyValue )
{
   ...
   return ( nPropertyValue == W_PROP_VIRTUAL_TAG )?true:false;
}
                  ...
                  uint32_t nCommandLength )
{
   ...
   /* Select */
   if (  ( pCommandBuffer[0] == 0x00 )
      && ( pCommandBuffer[1] == 0xA4 )
      && (( pCommandBuffer[3] == 0x00 ) || ( pCommandBuffer[3] == 0x0C ) ))
   {
   ...
   /* Read or Write */
   else if (  ( pCommandBuffer[0] == 0x00 )
      && (( pCommandBuffer[1] == 0xB0 ) || ( pCommandBuffer[1] == 0xD6 )))
   {
      if ( pVirtualTag->nState == P_VIRTUAL_TAG_STATE_ACTIVE )
      {
      ...
      /* Get the length */
      if((nCommandLength == 5) || ( pCommandBuffer[1] == 0xD6 ))
      {
...
continue_operation:
   ...
   else /* Write */
   {
      if( nLength <= P_VIRTUAL_TAG_MAX_WRITE_LC_VALUE)
      {
         ...
         {
            if ( pVirtualTag->bIsLocked == false )
            {
               CMemoryCopy(
                  &pVirtualTag->pNDEFFile[nOffset],
                  &pCommandBuffer[5],
            ...
            W_HANDLE* phHandle )
{
   ...
   switch ( nTagType )
   {
      case W_PROP_NFC_TAG_TYPE_4_A:
         ...
         if (PEmulIsPropertySupported(pContext, nTagType) == false)
         {
...
/* See Client API Specifications */
void WVirtualTagStart(
      W_HANDLE hVirtualTag,
      tWBasicGenericCallbackFunction* pCompletionCallback,
      ...
      bool bReadOnly)
{
   ...
   tDFCExternal dfc1;
   tDFCExternal dfc2;
   dfc1.pFunction = (tDFCCallback*)pCompletionCallback;
   dfc1.pParameter = pCompletionCallbackParameter;
   ...
   PVirtualTagStart( pContext, hVirtualTag, null, &dfc1, null, &dfc2, bReadOnly );
                  ...
                  bool bReadOnly )
{
   ...
   switch ( pVirtualTag->nTagType )
   {
      case W_PROP_NFC_TAG_TYPE_4_A:
       ...
       sEmulConnectionInfo.nCardType = W_PROP_ISO_14443_4_A;
         ...
         sEmulConnectionInfo.sCardInfo.s14A.bSetCIDSupport = false;
      ...
      case W_PROP_NFC_TAG_TYPE_4_B:
       ...
       sEmulConnectionInfo.nCardType = W_PROP_ISO_14443_4_B;
         ...
         sEmulConnectionInfo.sCardInfo.s14B.bSetCIDSupport = false;
   ...
   if (nError != W_SUCCESS)
   {
      ...
      PDFCPost2(pContext, P_DFC_TYPE_VIRTUAL_TAG, static_PVirtualTagCardEmulationOpenCompleted, pVirtualTag, nError);
      return;
   }
