 ...
 *
 * @param[in]  bCheckResponseCRC  The check CRC flag, if set to true, the response
 ...
 * @param[in]  nCardToReaderBufferMaxLength   The maximum card to reader buffer length.
 *
 * @param[in]  bSendNAD  The NAD is present flag. If true, NAD is present in the outgoing RF frame
 *
...
W_ERROR PSEDriverGetInfo(
            tContext* pContext,
            uint32_t nSlotIdentifier,
            OPEN_NFC_BUF1_O tSEInfo* pSEInfo,
            OPEN_NFC_BUF1_LENGTH uint32_t nSize );
...
/*******************************************************************************
   Contains the UICC functions
*******************************************************************************/

/**
 * See WUICCSetAccessPolicy()
 *
 * @keyword  DRIVER_API
 **/
void PUICCDriverSetAccessPolicy(
            tContext* pContext,
            uint32_t nStorageType,
            OPEN_NFC_BUF1_I const tWUICCAccessPolicy* pAccessPolicy,
            OPEN_NFC_BUF1_LENGTH uint32_t nSize,
            OPEN_NFC_USER_CALLBACK tPBasicGenericCallbackFunction* pCallback,
            void* pCallbackParameter);
...
W_ERROR PUICCDriverGetAccessPolicy(
            tContext* pContext,
            uint32_t nStorageType,
            OPEN_NFC_BUF1_O tWUICCAccessPolicy* pAccessPolicy,
            OPEN_NFC_BUF1_LENGTH uint32_t nSize);
...
void PUICCGetSlotInfoDriver(
                  tContext * pContext,
                  OPEN_NFC_USER_CALLBACK tPUICCGetSlotInfoCompleted* pCallback,
                  void* pCallbackParameter );
...
W_ERROR PP2PCreateSocketDriver(
            tContext * pContext,
            uint8_t nType,
            OPEN_NFC_BUF1_I const tchar * pServiceURI,
            OPEN_NFC_BUF1_LENGTH uint32_t  nSize,
            uint8_t nSAP,
