...
static bool static_PSmartCacheReadStateMachine(
               tContext* pContext,
               tSmartCache* pSmartCache);

static bool static_PSmartCacheWriteStateMachine(
               tContext* pContext,
               tSmartCache* pSmartCache);
            ...
            W_ERROR nError)
{
   ...
   else
   {
      ...
      if(static_PSmartCacheReadStateMachine(pContext, pSmartCache) != false)
      {
         return;
      }
      ...
      else
      {
         ...
         if(static_PSmartCacheWriteStateMachine(pContext, pSmartCache) != false)
         {
            return;
         }
      }
 ...
 *
 * @return  true if an operation is pending, false if the read operation is completed.
 **/
static bool static_PSmartCacheReadStateMachine(
               tContext* pContext,
               tSmartCache* pSmartCache)
{
   ...
   for(nFunctionIndex = 0; nFunctionIndex < P_SMART_CACHE_READ_FUNCTION_NUMBER; nFunctionIndex++)
   {
      ...
      {
         if(pSmartCache->pDescriptor->aReadFunctionArray[nBestFunctionIndex].nSectorNumber >
 ...
 * @param[in]  pSmartCache  The smart cache instance.
 *
 * @return  true if an operation is pending, false if the write operation is completed.
 **/
static bool static_PSmartCacheWriteStateMachine(
               tContext* pContext,
               tSmartCache* pSmartCache)
