...
#define P_NDEF_COMMAND_MAX_LENGTH            0x2800/*(256 + 5)*/
...
#define P_NDEF_COMMAND_READ_NDEF_SPLIT       0x03
#define P_NDEF_COMMAND_WRITE_NDEF            0x04
#define P_NDEF_COMMAND_WRITE_NDEF_LENGTH     0x05
#define P_NDEF_COMMAND_LOCK_TAG              0x06
#define P_NDEF_COMMAND_MOVE_READ             0x07
#define P_NDEF_COMMAND_MOVE_WRITE            0x08

/* NDEF read or write actions */
#define P_NDEF_ACTION_RESET                  0x00
#define P_NDEF_ACTION_READ                   0x01
#define P_NDEF_ACTION_READ_SPLIT             0x02
#define P_NDEF_ACTION_WRITE                  0x03
...
typedef struct __tNDEFConnection
{
   ...
   bool                       bIsLocked;                       /* TAG is locked ? */
   bool                       bIsLockable;                     /* TAG is lockable ? */
   bool                       bCompressionSupported;           /* TAG supports compression ? */

   uint8_t                    aCCFile[P_NDEF_MAX_CC_LENGTH];   /* CC contents */
   uint32_t                   nCCLength;                       /* CC length */

   uint8_t                    aSerialNumber[100];              /* serial number of the TAG */
   uint8_t                    nSerialNumberLength;             /* length of the serial number */
   ...
   uint8_t                    nByteLength;                     /* For type 1 and Type 2, contains the size in bytes of the length item of the NDEF TLV (1 or 3 bytes) */
   ...
   /* Command buffer */
   uint8_t                    aSendBuffer[P_NDEF_COMMAND_MAX_LENGTH];
   /* Response buffer */
   uint8_t                    aReceivedBuffer[P_NDEF_MAX_LENGTH];
   /* Response data length in bytes */
   uint32_t                   nReceivedDataLength;
   /* Response data offset in bytes */
   uint32_t                   nReceivedDataOffset;


   /* read opeation */

   uint8_t                    aBuffer[P_NDEF_MAX_LENGTH];                     /* used for NDEF storage */

   uint8_t                    nTNF;                                           /* TNF to found */
   tchar                      aTypeString[P_NDEF_MAX_STRING_LENGTH];          /* Type string to find */
   tchar                      aIdentifierString[P_NDEF_MAX_STRING_LENGTH];    /* Identifier string to found */

   bool                       bIsTNFFound;         /* when using READ_NDEF_SPLIT, indicates we have found a record with the correct TNF,
   ...
   union
   {
      ...
      struct
      {
         bool                       bIsDecompressedDataAvailable;
         uint32_t                   nLen;
      } t5;
 ...
 **/
bool PNDEFType4GetCapabilityContainer(
            tContext* pContext,
            tNDEFConnection* pNDEFConnection,
            uint32_t nLength );
