         ...
         void* pCallbackParameter )
{
   ...
   PMultiTimerPoll( pContext, false );
   ...
   for(nTimer = 0; nTimer < P_MULTI_TIMER_NUMBER; nTimer++)
   {
      ...
      if((nExpirationTime = pElement->nAbsoluteTimeout) != 0)
      {
         ...
         if (nDelta <= 0x7FFFFFFF)
         {
            ...
            if(pElement->pCallbackFunction != null)
            {
               PDFCPost1( pContext, pElement,
                  pElement->pCallbackFunction, pElement->pCallbackParameter );
            }
            else
            {
               /* User timer */
               PDFCDriverPostCC1(pElement->pDriverCC, pElement);
            }
                  ...
                  void* pCallbackParameter )
{
   ...
   if((nTimerIdentifier != TIMER_T12_USER_WJED)
   && (nTimerIdentifier != TIMER_T13_USER_TEST))
   {
