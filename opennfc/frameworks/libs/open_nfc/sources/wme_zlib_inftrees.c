...
const char WZLIB_inflate_copyright[] =
 ...
 */
sint32_t WZLIB_inflate_table(codetype type, uint16_t  *lens, uint32_t codes,
                  code  *  *table, uint32_t  *bits, uint16_t  *work)
{
    ...
    sint32_t left;                   /* number of prefix codes available */
    ...
    sint32_t end;                    /* use base and extra for symbol > end */
    ...
    /*
       ...
       symbols by length from short to sint32_t, and retaining the symbol order
    ...
    for (;;) {
        ...
        if ((sint32_t)(work[sym]) < end) {
            this.op = (uint8_t)0;
            this.val = work[sym];
        }
        else if ((sint32_t)(work[sym]) > end) {
