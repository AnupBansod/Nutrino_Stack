...
local sint32_t updatewindow (z_streamp strm, uint32_t out);
#ifdef BUILDFIXED
   void makefixed ((void));
#endif
local uint32_t syncsearch (uint32_t  *have, uint8_t  *buf,
                              uint32_t len);

sint32_t  WZLIB_inflateReset(z_streamp strm)
{
...
sint32_t  WZLIB_inflatePrime(z_streamp strm, sint32_t bits, sint32_t value)
{
    struct inflate_state  *state;
...
sint32_t  WZLIB_inflateInit2_(z_streamp strm, sint32_t windowBits, const char *version, sint32_t stream_size)
{
    struct inflate_state  *state;
    ...
    state->window = Z_NULL;
    return WZLIB_inflateReset(strm);
}

sint32_t  WZLIB_inflateInit_(z_streamp strm, const char *version, sint32_t stream_size)
{
    return WZLIB_inflateInit2_(strm, DEF_WBITS, version, stream_size);
}
...
#  define UPDATE(check, buf, len) \
    (state->flags ? crc32(check, buf, len) : WZLIB_adler32(check, buf, len))
#else
#  define UPDATE(check, buf, len) WZLIB_adler32(check, buf, len)
#endif
...
sint32_t  WZLIB_inflate(z_streamp strm, sint32_t flush)
{
    struct inflate_state  *state;
    ...
    for (;;)
        ...
        case HEAD:
            ...
            if (len > state->wbits) {
                ...
                state->mode = BAD;
                PDebugTrace("compressed windows size ( %d ) greater than compressed windows size (%d)\n", len, state->wbits);
                break;
            }
            state->dmax = 1U << len;
            Tracev((stderr, "inflate:   zlib header ok\n"));
            strm->adler = state->check = WZLIB_adler32(0L, Z_NULL, 0);
        ...
        case FLAGS:
            ...
            if (state->head != Z_NULL)
                state->head->text = (sint32_t)((hold >> 8) & 1);
        ...
        case OS:
            NEEDBITS(16);
            if (state->head != Z_NULL) {
                state->head->xflags = (sint32_t)(hold & 0xff);
                state->head->os = (sint32_t)(hold >> 8);
            }
            if (state->flags & 0x0200) CRC2(state->check, hold);
            INITBITS();
            state->mode = EXLEN;
        ...
        case HCRC:
            ...
            if (state->head != Z_NULL) {
                state->head->hcrc = (sint32_t)((state->flags >> 9) & 1);
                state->head->done = 1;
            }
...
sint32_t  WZLIB_inflateEnd(z_streamp strm)
{
    struct inflate_state  *state;
...
sint32_t  WZLIB_inflateSetDictionary(z_streamp strm, const Byte *dictionary, uInt dictLength)
{
    struct inflate_state  *state;
    uint32_t id;
    ...
    if (state->mode == DICT) {
        id = WZLIB_adler32(0L, Z_NULL, 0);
        id = WZLIB_adler32(id, dictionary, dictLength);
        if (id != state->check)
            return Z_DATA_ERROR;
    }
...
sint32_t  WZLIB_inflateGetHeader(z_streamp strm, gz_headerp head)
{
    struct inflate_state  *state;
 ...
 */
local uint32_t syncsearch (uint32_t  *have, uint8_t  *buf, uint32_t len)
{
    uint32_t got;
    uint32_t next;
    ...
    while (next < len && got < 4) {
        if ((sint32_t)(buf[next]) == (got < 2 ? 0 : 0xff))
            got++;
        else if (buf[next])
            got = 0;
        else
...
sint32_t  WZLIB_inflateSync(z_streamp strm)
{
 ...
 */
sint32_t  WZLIB_inflateSyncPoint(z_streamp strm)
{
    struct inflate_state  *state;
...
sint32_t  WZLIB_inflateCopy(z_streamp dest, z_streamp source)
{
    struct inflate_state  *state;
    ...
    copy->window = window;
    dest->state = (struct deflate_state  *)copy;
    return Z_OK;
}
