   ...
   )
{
   ...
   if (pConnection->nState == LLCP_CONNECTION_STATE_IDLE)
   {
      ...
      if (nURILength != 0)
      {
         ...
         pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sSN.bIsPresent = true;
   ...
   W_ERROR nError)
{
   ...
   PP2PDisconnectCompleteCallback(pContext, pConnection, nError);
   ...
   tLLCPConnection * pConnection)
{
   ...
   {
      /* nominal case */
      pConnection->bShutdownRequested = true;
 ...
 *
 * @return - true if a PDU has been sent, otherwise false
 *
 */

bool PLLCPConnectionStartXmitSDU(
   tContext          * pContext,
   tLLCPConnection   * pConnection
)
{
   ...
   if ((pConnection->bIsPeerBusy == true) && (pConnection->bShutdownRequested == false))
   {
