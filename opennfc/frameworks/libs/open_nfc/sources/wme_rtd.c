...
{
   return ( ((pBuffer[0]&0x80)==0x00) ? true : false );
}
                         ...
                         uint32_t nLanguageLength)
{
   ...
   for (i=0; i<(nLanguageLength-1); i++)
   {
      ...
      else if (*(pLanguage + i) != *(pLanguage1 + i) && bFlag == false)
      {
         nResult = 2;
         break;
      }
      ...
      if (*(pLanguage1 + i + 1) == '-' && i == nLanguageLength - 2 && bFlag == false)
      {
         nResult = 1;
      }
   }
         ...
         uint32_t * pBufferLength )
{
   ...
   if (PNDEFCheckType(pContext, hRecord, W_NDEF_TNF_WELL_KNOWN, pURI) == false)
   {
   ...
   nSize = nIdCodeLength +  PUtilConvertUTF8ToUTF16( null, &pPayloadBuffer[1], nPayloadSize - 1);

   if ((pTemp = CMemoryAlloc((nSize+1)* sizeof(tchar))) == null)
   {
         ...
         uint32_t nBufferLength )
{
   ...
   if (PRTDIsTextRecord( pContext, hRecord) == false)
   {
   ...
   *(pLanguageBuffer + nLanguageLength) = '\0';
   ...
   {
      *(pLanguageBuffer + nLanguageLength) = (tchar)(*(pRecord->pRecordBuffer + nPayloadOffset + nLanguageLength));
   }
   return W_SUCCESS;
}
         ...
         uint8_t* pnMatch )
{
   ...
   if ( PRTDIsTextRecord( pContext, hCurrentRecord) == false )
   {
         ...
         uint32_t nBufferLength )
{
   ...
   else
   {
      ...
      CMemoryCopy( (uint8_t *)pBuffer, pText, nTextLength );
      pBuffer[ nTextLength / sizeof(tchar) ] = 0;
      return W_SUCCESS;
   }
}
         ...
         W_HANDLE* phRecord )
{
   ...
   else
   {
      ...
      nTotalLengthInByte += nTextLength * sizeof(tchar);
   }
   ...
   if ((bUseUtf8 == false) && ((nLanguageLength & 1) == 0))
   {
   ...
   * pBuffer  = (uint8_t) nLanguageLength;
   ...
   if (bUseUtf8 == false)
   {
      PUtilConvertUTF16ToUTF16BE((tchar *) pBuffer, pText, nTextLength);
   }
   else
   {
         ...
         W_HANDLE* phRecord )
{
   ...
   if(pURI != null)
   {
      if (PNDEFUtilStringIsPrintableURI(pURI) == false)
      {
         return (W_ERROR_BAD_PARAMETER);
      }
