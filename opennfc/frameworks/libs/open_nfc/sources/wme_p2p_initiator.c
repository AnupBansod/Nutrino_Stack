...
tHandleType g_sP2PInitiatorDriverConnection = { static_PP2PDriverDestroyConnection, null, null, null, null };
         ...
         uint32_t nReceptionCounter )
{
   ...
   PDFCPostContext2(& pP2PDriverConnection->sCallbackContext, P_DFC_TYPE_P2P,  nError );
         ...
         uint32_t nLength)
{
   ...
   if ((pPDU = PLLCPFramerGetNextXmitPDU(pContext, true)) != null)
   {
      ...
      bool bCardApplicationMatch )
{
   ...
   if (nError == W_SUCCESS)
   {
      pPDU = PLLCPFramerGetNextXmitPDU(pContext, true);
...
)
{
   ...
   if (pP2PInstance->sLLCPInstance.bActivateInitiator)
   {
   ...
   {
      PDebugError("static_PP2PInitiatorStartDetection : nError %s\n", PUtilTraceError(nError));
   ...
   )
{
   ...
   if (pP2PInstance->hP2PInitiatorConnection != W_NULL_HANDLE)
   {
      ...
      PReaderDriverWorkPerformed(pContext, pP2PInstance->hP2PInitiatorConnection, false, true);
