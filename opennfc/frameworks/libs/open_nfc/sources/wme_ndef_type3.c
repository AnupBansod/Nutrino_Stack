...

{
   ...
   /* Check the type */
   if ( ( nError = PFeliCaUserCheckType(pContext, pNDEFConnection->hConnection, pNDEFConnection->aSerialNumber, & pNDEFConnection->nSerialNumberLength ) ) != W_SUCCESS )
   {
            ...
            tNDEFConnection* pNDEFConnection )
{
   ...
   pNDEFConnection->sType.t3.bIsChecksumCorrect = false;
   ...
   pNDEFConnection->sType.t3.bIsChecksumCorrect = true;
   ...
   else
   {
      ...
      if (pNDEFConnection->sType.t3.nCurrentOperation == P_NDEF_COMMAND_READ_NDEF)
      {
         nError2 = PNDEFSendCommandCompleted( pContext, pNDEFConnection,
                     pNDEFConnection->aReceivedBuffer + pNDEFConnection->sType.t3.nBegin % P_NDEF_3_BLOCK_SIZE,
...
static void static_PNDEFtype3ReadFirstBlockCompleted(tContext* pContext, void* pCallbackParameter, W_ERROR nError )
{
   ...
   CMemoryCopy(pNDEFConnection->aReceivedBuffer + pNDEFConnection->sType.t3.nBegin % P_NDEF_3_BLOCK_SIZE, pNDEFConnection->pMessageBuffer, P_NDEF_3_BLOCK_SIZE);
               ...
               uint32_t nLength)
{
   ...
   if (pBuffer == null)
   {
      ...
      PDFCPost2(pContext, P_DFC_TYPE_TYPE3, static_PNDEFtype3ReadWriteNDEFSliceCompleted , pNDEFConnection, W_ERROR_OUT_OF_RESOURCE);
      return;
   }
   ...
   if (pNDEFConnection->sType.t3.nCurrentOperation == P_NDEF_COMMAND_READ_NDEF)
   {
      ...
      PFeliCaRead(
         ...
         static_PNDEFtype3ReadWriteNDEFSliceCompleted,
         pNDEFConnection,
         pNDEFConnection->aReceivedBuffer + pNDEFConnection->sType.t3.nBytesProcessed,
