            ...
            W_ERROR nError )
{
   ...
   CMemoryCopy(pNDEFConnection->aCCFile,
               pNDEFConnection->aReceivedBuffer + P_NDEF_2_CC_OFFSET,
               P_NDEF_2_CC_LENGTH);
   ...
   if ( pNDEFConnection->nNDEFId != 0xFFFF )
   {
      ...
      if ((nError == W_SUCCESS) && (PBasicCheckConnectionProperty(pContext, pNDEFConnection->hConnection, W_PROP_MIFARE_UL_C) == W_SUCCESS))
      {
         /* A Mifare UL C card has been found, retreive the access rights */
         PMifareULRetrieveAccessRights(pContext, pNDEFConnection->hConnection, static_PMifareULRetrieveAccessRightsCompleted, pNDEFConnection);
      }
      else
      {
         /* Send the result */
            ...
            tNDEFConnection* pNDEFConnection )
{
   ...
   {
      PMifareCheckType2(pContext, pNDEFConnection->hConnection, null, null, &pNDEFConnection->bIsLocked, &pNDEFConnection->bIsLockable, null, null, 0);
   }
   else if ( (PBasicCheckConnectionProperty(pContext, pNDEFConnection->hConnection, W_PROP_MY_D_MOVE) == W_SUCCESS) ||
             (PBasicCheckConnectionProperty(pContext, pNDEFConnection->hConnection, W_PROP_MY_D_NFC) == W_SUCCESS))
   {
      PMyDCheckType2(pContext, pNDEFConnection->hConnection, null, null, &pNDEFConnection->bIsLocked, &pNDEFConnection->bIsLockable, null, null, 0);
   }
   else
   {
      pNDEFConnection->bIsLocked = ( (pNDEFConnection->aReceivedBuffer[P_NDEF_2_LOCK_OFFSET + 0] != 0x00) ||
                                     (pNDEFConnection->aReceivedBuffer[P_NDEF_2_LOCK_OFFSET + 1] != 0x00) );
...
static bool static_PNDEFType2ReadWriteAreaAutomaton(tContext* pContext, tNDEFConnection* pNDEFConnection);
            ...
            uint32_t nLength )
{
   ...
   switch ( pNDEFConnection->nCommandType )
   {
      ...
      case P_NDEF_COMMAND_READ_NDEF_SPLIT:
         ...
         {
            PDebugTrace("static_PNDEFType2SendCommand : P_NDEF_COMMAND_READ_NDEF_SPLIT");
         }
         ...
         pNDEFConnection->sType.t2.pBuffer = pNDEFConnection->aReceivedBuffer;
      ...
      case P_NDEF_COMMAND_WRITE_NDEF_LENGTH:
         ...
         if (pNDEFConnection->nByteLength == 1)
         {
            ...
            pNDEFConnection->aSendBuffer[0] = (uint8_t) pNDEFConnection->nNDEFFileLength;
            pNDEFConnection->sType.t2.nBytesToProcess = 1;
         }
         else
         {
            pNDEFConnection->aSendBuffer[0] = 0xFF;
            pNDEFConnection->aSendBuffer[1] = (uint8_t) (pNDEFConnection->nNDEFFileLength >> 8);
            pNDEFConnection->aSendBuffer[2] = (uint8_t) (pNDEFConnection->nNDEFFileLength);
            pNDEFConnection->sType.t2.nBytesToProcess = 3;
         }

         pNDEFConnection->sType.t2.nBytesProcessed  = 0;
         pNDEFConnection->sType.t2.nBytesPending  = 0;
         pNDEFConnection->sType.t2.pBuffer = pNDEFConnection->aSendBuffer;
 ...
 */
static bool static_PNDEFType2ReadWriteAreaAutomaton(
   tContext* pContext,
   tNDEFConnection* pNDEFConnection)
{
   uint32_t nBegin;
   ...
   switch (pNDEFConnection->sType.t2.nCommandWriteState)
   {
      ...
      case P_NDEF_2_WRITE_CC :
         ...
         if (pNDEFConnection->sType.t2.nDynamicLockSize != 0)
         {
            ...
            CMemoryFill(pNDEFConnection->aSendBuffer, 0xFF, pNDEFConnection->sType.t2.nDynamicLockSize);
            /* All unused bits in the last byte shall always be set to 0 */
            pNDEFConnection->aSendBuffer[pNDEFConnection->sType.t2.nDynamicLockSize - 1] >>= pNDEFConnection->sType.t2.nDynamicLockSize * 8 - pNDEFConnection->sType.t2.nDynamicLockBits;
...
static void static_PNDEFType2MoveAutomaton(tContext* pContext, void* pCallbackParameter, W_ERROR nError)
{
   ...
   switch (pNDEFConnection->sType.t2.nCommandWriteState)
   {
      case P_NDEF_2_MOVE_START :
         ...
         CMemoryFill(pNDEFConnection->aReceivedBuffer, 0, (pNDEFConnection->nByteLength == 1) ? 3 : 1);
         /* Bytes to move are read in tag */
         pNDEFConnection->sType.t2.pBuffer = &pNDEFConnection->aReceivedBuffer[(pNDEFConnection->nByteLength == 1) ? 3 : 1];
...
static W_ERROR static_PNDEFType2InvalidateCache(
   ...
   uint32_t nLength)
{
   uint32_t nBegin = nOffset + pNDEFConnection->nNDEFId + 1 + pNDEFConnection->nByteLength;
   uint32_t nEnd;
   W_ERROR  nError;
