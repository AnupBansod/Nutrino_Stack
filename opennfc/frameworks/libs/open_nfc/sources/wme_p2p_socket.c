...
)
{
   ...
   if (nURILength)
   {
      return PP2PCreateSocketDriver(pContext, nType, pServiceURI, (nURILength + 1) * sizeof(tchar), nSAP, phSocket);
   }
   else
   {
...
static uint32_t static_PP2PDestroySocket(tContext* pContext, void* pObject )
{
   ...
   if (pP2PSocket->sConfig.bServiceRegistered == true)
   {
...
tHandleType g_sP2PSocket = { static_PP2PDestroySocket, null, null, null, null};
...
)
{
   ...
   if ((nType == W_P2P_TYPE_SERVER) || (nType == W_P2P_TYPE_CLIENT_SERVER) || (nType == W_P2P_TYPE_CONNECTIONLESS))
   {
      ...
      if (PLLCPSDPCheckURIAndSAPConsistency(&pP2PInstance->sLLCPInstance.sSDPInstance, pUTF8ServiceURI, nUTF8ServiceURILength, nSAP) == false)
      {
      ...
      if (pUTF8ServiceURI != null)
      {
         ...
         pP2PSocket->sConfig.bServiceRegistered = true;
      }
   ...
   W_ERROR        nError)
{
   ...
   {
      pP2PSocket->bRecvInProgress = false;
      ...
      PDFCDriverPostCC3(pP2PSocket->pRecvCC, P_DFC_TYPE_P2P, 0, nError);
   }
   ...
   uint8_t       nSAP)
{
   ...
   if (pP2PSocket->sConfig.nType == W_P2P_TYPE_CONNECTIONLESS)
   {
      PDFCDriverPostCC4(pP2PSocket->pRecvCC, P_DFC_TYPE_P2P, nRecvLength, nError, nSAP);
   }
   else
   {
      PDFCDriverPostCC3(pP2PSocket->pRecvCC, P_DFC_TYPE_P2P, nRecvLength, nError);
   }
}
   ...
   tP2PSocket  * pP2PSocket)
{
   ...
   if ((pP2PSocket->sConnection.pPendingConnection != null) &&
       (pP2PSocket->sConnection.pPendingConnection != pP2PSocket->sConnection.pConnection))
   {
      PLLCPConnectionFreeContext(pContext, pP2PSocket->sConnection.pPendingConnection);
      pP2PSocket->sConnection.pPendingConnection = null;
   }

   if (pP2PSocket->sConnection.pConnection != null)
   {
      PLLCPConnectionFreeContext(pContext, pP2PSocket->sConnection.pConnection);
      pP2PSocket->sConnection.pConnection = null;
   }

   pP2PSocket->sConnection.bConnectInProgress = false;
   pP2PSocket->sConnection.bIsConnectCalled = false;
   pP2PSocket->sConnection.bIsConnected = false;
