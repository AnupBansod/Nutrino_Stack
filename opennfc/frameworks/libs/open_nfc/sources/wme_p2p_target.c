...
static void static_PP2PTargetLocalTimeoutExpiry(tContext * pContext, void * pCallbackParameter);
      ...
      W_ERROR    nError )
{
   ...
   if (nError != W_SUCCESS)
   {
      PDebugError("static_PP2PTargetDetectionStarted : nError %s\n", PUtilTraceError(nError));
...
/*
 *  P2P target Emulation local timeout expiry
 *  Send a SYMM
 */

void static_PP2PTargetLocalTimeoutExpiry(
   tContext * pContext,
   void     * pCallbackParameter
   )
{
   ...
   if (pPDU != null)
   {
      PEmulSendAnswer(pContext, pP2PInstance->hP2PTargetConnection, pPDU->aPayload, pPDU->nLength);
      PLLCPFreePDU(pContext, pPDU);
   }
   else
   {
