      ...
      W_ERROR nError )
{
   ...
   PDFCPostContext3(pCallbackContext, P_DFC_TYPE_P2P, (uint32_t) nDSAP, nError);
      ...
      const tchar * pServiceURI)
{
   ...
   if (pCallbackContext != null)
   {
      ...
      if (pServiceURI != null)
      {
         PP2PURILookupDriver(pContext, hLink, static_PP2PURILookupCompleted, pCallbackContext, pServiceURI, PUtilStringLength(pServiceURI) * sizeof(tchar));
      }
      else
      {
      /* all errors are handled in the driver implementation */
      ...
      if (nError != W_SUCCESS)
      {
         PDFCPost3(pContext, P_DFC_TYPE_P2P, static_PP2PURILookupCompleted, pCallbackContext, 0, nError);
      }
   }
   else
   {
      ...
      PDFCPostContext3(&sCallbackContext, P_DFC_TYPE_P2P, 0, W_ERROR_OUT_OF_RESOURCE);
   }
}
      ...
      uint32_t nLength)
{
   ...
   if ((pConfiguration->bActivateInitiator != false) && (pConfiguration->bActivateInitiator != true))
   {
      PDebugError("PP2PSetConfigurationDriver : bActivateInitiator must be true/false");
      return W_ERROR_BAD_PARAMETER;
   }

   pLLCPInstance->nLocalTimeoutMs = pConfiguration->nLocalLTO;
   pLLCPInstance->nLocalLinkMIU = pConfiguration->nLocalMIU;
   pLLCPInstance->bActivateInitiator = pConfiguration->bActivateInitiator;
      ...
      uint32_t nLength)
{
   ...
   pConfiguration->bActivateInitiator =  pLLCPInstance->bActivateInitiator;
   ...
   tP2PLink * pP2PLink)
{
   ...
   CDebugAssert(pP2PLink->bIsRegistered == true);
   ...
   W_ERROR    nError)
{
   ...
   if (pP2PLink->bEstablishedCalled == false)
   {
      ...
      if (nError == W_SUCCESS)
      {
         PDFCDriverPostCC3(pP2PLink->pEstablishmentCC, P_DFC_TYPE_P2P, pP2PLink->hLink, W_SUCCESS);
      }
      else
      {
         PDFCDriverPostCC3(pP2PLink->pEstablishmentCC, P_DFC_TYPE_P2P, W_NULL_HANDLE, nError);
         PDFCDriverFlushCall(pP2PLink->pReleaseCC);
      }

      pP2PLink->bEstablishedCalled = true;
   }
}
   ...
   W_ERROR    nError)
{
   ...
   if (pP2PLink->bEstablishedCalled == true)
   {
      PDFCDriverPostCC2(pP2PLink->pReleaseCC, P_DFC_TYPE_P2P, nError);
   }
   else
   {
      if (nError == W_SUCCESS)
   ...
   W_ERROR    nError)
{
   ...
   for (pP2PLink = pP2PInstance->pFirstP2PLink; pP2PLink != null; pP2PLink = pP2PInstance->pFirstP2PLink)
   {
      ...
      if (pP2PLink->bEstablishedCalled != false)
      {
         ...
         if (pP2PLink->bIsDestroyPending == false)
         {
            static_PP2PDecrementLLCPUsageCount(pContext);
         }
         else
...
static uint32_t static_PP2PDestroyLink( tContext* pContext, void* pObject )
{
   ...
   if (pP2PLink->bIsRegistered)
   {
      ...
      pP2PLink->bIsDestroyPending = true;
...
tHandleType g_sP2PLink = { static_PP2PDestroyLink, null, null, null, null } ;
         ...
         bool bIsClosing)
{
   ...
   if (bIsClosing == false)
   {
      ...
      if (pP2PLink->bIsRegistered == true)
      {
         static_PP2PUnregisterLink(pContext, pP2PLink);
         static_PP2PDecrementLLCPUsageCount(pContext);
      }
 ...
 )
{
   ...
   pP2PLink->pEstablishmentCC = pDriverCC;
...
error:
   ...
   PDFCDriverPostCC3(pDriverCC, P_DFC_TYPE_P2P, W_NULL_HANDLE, nError);
   ...
   W_HANDLE * phOperation)
{
   ...
   PDFCDriverFillCallbackContext( pContext, (tDFCCallback*)pReleaseCallback, pReleaseCallbackParameter, & pP2PLink->pReleaseCC);
      ...
      uint32_t nLength)
{
   ...
   pProperties->nBaudRate = 106000;                      /* @todo : hard-coded to 106000 waiting for NFC controller to support it */

   pProperties->bIsInitiator = (pLLCPInstance->nRole == LLCP_ROLE_INITIATOR) ? true : false;
   ...
   )
{
   ...
   while ((pPDU = PLLCPFramerGetNextXmitPDU(pContext, false)) != null)
   {
   ...
   uint32_t nSize)
{
   ...
   if (pP2PInstance->sLLCPInstance.bURILookupInProgress != false)
   {
   ...
   nUTF8ServiceLength = PUtilConvertUTF16ToUTF8(null, pServiceURI, nSize / sizeof(tchar));
   ...
   PUtilConvertUTF16ToUTF8(pUTF8ServiceURI, pServiceURI, nSize / sizeof(tchar));
   ...
   pLLCPInstance->bURILookupInProgress = true;
   ...
   sPDUDescriptor.sPayload.sParams.sSDREQ.bIsPresent = true;
