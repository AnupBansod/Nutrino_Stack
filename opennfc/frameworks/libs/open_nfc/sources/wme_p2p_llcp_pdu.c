...
static bool static_PLLCPDecodePDU(uint8_t * pPDU, uint16_t nPDULength, tDecodedPDU * pDecodedPDU);
...
tLLCPPDUHeader * PLLCPAllocatePDU(tContext * pContext)
{
   ...
   nPDULength = (uint16_t) (uint32_t) (((tLLCPPDUHeader *)0)->aPayload) + LLCP_NUMBERED_PDU_DATA_OFFSET + pLLCPInstance->nRemoteLinkMIU;

   pPDU = CMemoryAlloc(nPDULength);

   if (pPDU != null)
   {
      pPDU->nDataOffset = pPDU->nLength = 0;
      pPDU->pNext = null;
   ...
   )
{
   ...
   if (static_PLLCPDecodePDU(pRcvPDU->aPayload, pRcvPDU->nLength, &pLLCPInstance->sRcvPDUDescriptor) == false)
   {
   ...
   else
   {
      ...
      while (nProcessedLength < pRcvPDU->nLength)
      {
         ...
         if (static_PLLCPDecodePDU(pTempPDU->aPayload, nTLVLength-2, &pLLCPInstance->sRcvPDUDescriptor) == true)
         {
   ...
   )
{
   ...
   for (pCurrentParameter = pParameters; nTotalParamLength > 0; )
   {
      ...
      switch (nCurrentParameterType)
      {
         case LLCP_PARAM_VERSION_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sVersion.bIsPresent = true;
         ...
         case LLCP_PARAM_MIUX_ID  :
            ...
            {
               pDecodedPDU->sPayload.sParams.sMIUX.bIsPresent = true;
         ...
         case LLCP_PARAM_WKS_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sWKS.bIsPresent = true;
         ...
         case LLCP_PARAM_LTO_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sLTO.bIsPresent = true;
         ...
         case LLCP_PARAM_RW_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sRW.bIsPresent = true;
         ...
         case LLCP_PARAM_SN_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sSN.bIsPresent = true;
         ...
         case LLCP_PARAM_OPT_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sOPT.bIsPresent = true;
         ...
         case LLCP_PARAM_SDREQ_ID :
            ...
            {
               pDecodedPDU->sPayload.sParams.sSDREQ.bIsPresent = true;
         ...
         case LLCP_PARAM_SDRES_ID:
            ...
            {
               pDecodedPDU->sPayload.sParams.sSDRES.bIsPresent = true;
   ...
   switch (pDecodedPDU->nType)
   {
      case LLCP_PDU_PAX :  /* not supported, but used for negociated parameters parsing */
         ...
         pDecodedPDU->sPayload.sParams.sRW.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSN.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSDREQ.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSDRES.bIsPresent = false;
      ...
      case LLCP_PDU_CONNECT :
         ...
         pDecodedPDU->sPayload.sParams.sLTO.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sOPT.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sVersion.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sWKS.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSDREQ.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSDRES.bIsPresent = false;
         break;
      ...
      case LLCP_PDU_SNL :
            ...
            /* mark unsupported parameters as not present */
         pDecodedPDU->sPayload.sParams.sLTO.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sMIUX.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sOPT.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sRW.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sSN.bIsPresent = false;
         pDecodedPDU->sPayload.sParams.sVersion.bIsPresent = false;
...
bool static_PLLCPDecodePDU(uint8_t * pPDU, uint16_t nPDULength, tDecodedPDU * pDecodedPDU)
{
   bool bValid = false;    /* the bValue will be set to true once all checks passed */
   ...
   switch (pDecodedPDU->nType)
   {
      ...
      case LLCP_PDU_PAX :
         ...
         bValid = false;      /* PAX PDU is not supported when using the 18092 binding */
         break;
      ...
      case LLCP_PDU_DISC :
         ...
         bValid = (nPDULength == LLCP_UNUMBERED_PDU_MIN_LENGTH) ? true : false;
         break;
...
)
{
   ...
   if (pPDUDescriptor->sPayload.sParams.sVersion.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_VERSION_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sMIUX.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_MIUX_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sWKS.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_WKS_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sLTO.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_LTO_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sRW.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_RW_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sSN.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET;
      nLength += pPDUDescriptor->sPayload.sParams.sSN.nSNLength;
   }

   if (pPDUDescriptor->sPayload.sParams.sOPT.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + LLCP_PARAM_OPT_LENGTH;
   }

   if (pPDUDescriptor->sPayload.sParams.sSDREQ.bIsPresent == true)
   {
      nLength += LLCP_PARAM_DATA_OFFSET + 1;
      nLength += pPDUDescriptor->sPayload.sParams.sSDREQ.nSNLength;
   }

   if (pPDUDescriptor->sPayload.sParams.sSDRES.bIsPresent == true)
   {
...
)
{
   ...
   switch (pLLCPInstance->sRcvPDUDescriptor.nType)
   {
      case LLCP_PDU_CONNECT :
         ...
         if (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sSN.bIsPresent == true)
         {
      ...
      case LLCP_PDU_SNL :
             ...
             (pLLCPInstance->nAgreedVersionMinor >= LLCP_VERSION_11_MINOR))
         {
            ...
            if (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sSDREQ.bIsPresent == true)
            {
               ...
               pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sSDRES.bIsPresent = true;
            ...
            if (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sSDRES.bIsPresent == true)
            {
               /* process a name resolution response */

               if ( (pLLCPInstance->bURILookupInProgress == true)
                    && (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sSDRES.nTID == pLLCPInstance->nCurrentTID))
               {
                  PDFCDriverPostCC3(pLLCPInstance->pURILookupCC, P_DFC_TYPE_P2P, (uint32_t) pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sSDRES.nSAP, W_SUCCESS);

                  pLLCPInstance->bURILookupInProgress = false;
