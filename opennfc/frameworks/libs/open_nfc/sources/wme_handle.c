         ...
         tHandleList* pHandleList )
{
   if(pHandleList != null)
   {
      ...
      for(nPosition = 0; nPosition < pHandleList->nListSize; nPosition++)
      {
         ...
         else if(P_HANDLE_IS_STATE_FREE(nState) == false)
         {
            tHandleObjectHeader* pObjectHeader = pListElement->pObjectHeader;
            ...
            W_HANDLE* phHandle)
{
   ...
   return static_PHandleAddEntry( pContext, (tHandleObjectHeader*)pObject, false, phHandle);
}
...
static bool static_PHandleIsValid(
                     tContext* pContext,
                     W_HANDLE hObject)
{
   uint32_t nPosition = static_PHandleGetObjectPosition(pContext, hObject);

   return (nPosition == ((uint32_t)-1)) ? false : true;
}
            ...
            tHandleType* pType)
{
   ...
   for(nPosition = 0; nPosition < pHandleList->nListSize ; ++nPosition)
   {
      if((P_HANDLE_IS_STATE_FREE(pHandleList->pElementList[nPosition].nState) == false)
      && (P_HANDLE_IS_STATE_DESTROY_PENDING(pHandleList->pElementList[nPosition].nState) == false)
            ...
            void** ppObject)
{
   ...
   *ppObject = static_PHandleGetObject(pContext, hObject, pExpectedType);
   ...
   if(*ppObject == null)
   {
      PDebugError("PHandleGetObject: Illegal value for the handle %08X", hObject);
      return W_ERROR_BAD_HANDLE;
   }
            ...
            void** ppObject)
{
   ...
   if (static_PHandleIsValid(pContext, hHandle) == false)
   {
      return W_ERROR_BAD_HANDLE;
   }
...
W_ERROR PHandleDuplicate(
            ...
            W_HANDLE* phNewHandle )
{
   return static_PHandleDuplicate(pContext, hObject, phNewHandle, false);
}
...
W_ERROR PHandleDuplicateWeak(
            ...
            W_HANDLE* phNewHandle )
{
   return static_PHandleDuplicate(pContext, hObject, phNewHandle, true);
}
            ...
            W_ERROR nError)
{
   ...
   while( nPosition != ((uint32_t) -1 ))
   {
      ...
      {
         PDFCPostContext2(pElement->pCloseSafeCallbackContext, P_DFC_TYPE_HANDLE, nError);
...
bool PHandleIsUser(
            ...
            W_HANDLE hObject )
{
   return (hObject & P_HANDLE_DRIVER_MARKER_BIT) == 0 ? true : false;
}
            ...
            W_HANDLE hObject )
{
   ...
   if(hObject != W_NULL_HANDLE)
   {
      ...
      if (PHandleIsUser(pContext, hObject) == false)
      {
      ...
      if(P_HANDLE_IS_STATE_WEAK(nState) == false)
      {
            ...
            W_ERROR nError)
{
   ...
   PDFCPostContext2(pCallbackContext, P_DFC_TYPE_HANDLE, nError);
...
send_result:
   ...
   PDFCPostContext2(&sCallbackContext, P_DFC_TYPE_HANDLE, nError);
}
   ...
   *pnPropertyNumber = 0;
   ...
   if (null != pObjectHeader->pType->pGetPropertyNumber)
   {
      ...
      if ( (nNumber > 0) && (null != pObjectHeader->pType->pGetProperties) )
      {
         ...
         if (false == pObjectHeader->pType->pGetProperties(pContext, pObjectHeader, pProperties))
         {
            PDebugError("static_PHandleGetVisibleProperties: Error returned by the get property function");
         ...
         for (i = 0; i < nNumber; i++)
         {
            if (true == PReaderUserIsPropertyVisible(pProperties[i]))
            {
...
#  endif
   ...
   do
   {
...
#endif /* P_CONFIG_USER || P_CONFIG_MONOLITHIC */
      {
         ...
         {
            if(pObjectHeader->pType->pGetProperties(pContext, pObjectHeader, pPropertyArray) == false)
            {
...
#  endif
   ...
   do
   {
      ...
      {
         if(pObjectHeader->pType->pCheckProperties(pContext, pObjectHeader, nPropertyValue) != false)
         {
            return W_SUCCESS;
         }
      }
