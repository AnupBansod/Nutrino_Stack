...
/* Get properties connection callback */
static bool static_PFeliCaUserGetProperties(
            tContext* pContext,
            void* pObject,
            uint8_t* pPropertyArray );

/* Check properties connection callback */
static bool static_PFeliCaUserCheckProperties(
            tContext* pContext,
            void* pObject,
            ...
            uint32_t nCardToReaderBufferMaxLength )
{
   ...
   if (nError != W_SUCCESS)
   {
      PDFCPost3(pContext, P_DFC_TYPE_FELICA, pCallback, pCallbackParameter, 0, nError);
   }
}
            ...
            W_ERROR nError )
{
   ...
   pFeliCaUserConnection->nError = nError;

   PDFCPostContext2( &pFeliCaUserConnection->sCallbackContext, P_DFC_TYPE_FELICA, nError );
}
...
send_result:
   ...
   static_PFeliCaUserSendError(pFeliCaUserConnection, nError);
}
            ...
            uint8_t* pnSerialNumberLength)
{
   ...
   {
      CMemoryCopy(pSerialNumber, pFeliCaUserConnection->aManufacturerID, P_FELICA_MANUFACTURER_ID_LENGTH);
   }

   if (pnSerialNumberLength != null)
   {
      * pnSerialNumberLength = P_FELICA_MANUFACTURER_ID_LENGTH;
   }
...
/* See tWBasicGenericDataCallbackFunction */
static void static_PFeliCaReadDataCompleted(
            tContext* pContext,
            void *pCallbackParameter,
            uint32_t nDataLength,
            W_ERROR nError)
{
   ...
   if ( nError != W_SUCCESS)
   {
      PDebugError("static_PFeliCaReadDataCompleted : nError %d\n", nError);
      goto send_result;
   }
   ...
   {
      PDebugError("static_PFeliCaReadDataCompleted : invalid answer format");
      nError = W_ERROR_CONNECTION_COMPATIBILITY;
   }
            ...
            W_HANDLE* phOperation )
{
   ...
   uint8_t i,nBlockListElementLength;
   ...
   if ( nError != W_SUCCESS )
   {
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, nError );
      return;
   }

   /* Check if an operation is still pending */
   if ( pFeliCaUserConnection->nError == P_ERROR_STILL_PENDING )
   {
      PDebugError("PFeliCaRead: W_ERROR_BAD_STATE");

      /* Send the error */
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, W_ERROR_BAD_STATE );
      return;
   }
       ...
       (nLength < (uint32_t)(nNumberOfBlocks * P_FELICA_BLOCK_SIZE)))
   {
      ...
      /* Send the error */
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, W_ERROR_BAD_PARAMETER );
      return;
   }
   ...
   for (i=0;i<nNumberOfBlocks;i++)
   {
      if ((pTempBuffer[0] & P_FELICA_LEN_MASK) == 0)
      {
         ...
         /* block number stored in LSB */
         nBlockNumber = pTempBuffer[1] + (pTempBuffer[2] << 8);
      }
      else
      {
         /* two bytes block */
   ...
   if ( pFeliCaUserConnection->nProtocol == P_PROTOCOL_STILL_UNKNOWN )
   {
      ...
      /* Send the error */
      static_PFeliCaUserSendError( pFeliCaUserConnection, W_ERROR_CONNECTION_COMPATIBILITY );
      return;
   }

   /* Reset the error code */
   pFeliCaUserConnection->nError             = P_ERROR_STILL_PENDING;
   ...
   /* Send the command */
   static_PFeliCaDriverExchangeData(pContext, pFeliCaUserConnection->hDriverConnection,
                           static_PFeliCaReadDataCompleted, pFeliCaUserConnection,
                           pFeliCaUserConnection->pReaderToCardBuffer, nOffset,
                           pFeliCaUserConnection->pCardToReaderBuffer,
                           NAL_MESSAGE_MAX_LENGTH );
...
/* See tWBasicGenericDataCallbackFunction */
static void static_PFeliCaWriteDataCompleted(
            tContext* pContext,
            void *pCallbackParameter,
            uint32_t nDataLength,
            W_ERROR nError)
{
   ...
   if ( nError != W_SUCCESS)
   {
      PDebugError("static_PFeliCaWriteDataCompleted : nError %d\n", nError);
      goto send_result;
   }
   ...
   {
      PDebugError("static_PFeliCaWriteDataCompleted : invalid answer format");
      nError = W_ERROR_CONNECTION_COMPATIBILITY;
   }
            ...
            W_HANDLE* phOperation )
{
   ...
   if ( pFeliCaUserConnection->nError == P_ERROR_STILL_PENDING )
   {
      PDebugError("PFeliCaWrite: W_ERROR_BAD_STATE");
   ...
   /* Send the command */
   static_PFeliCaDriverExchangeData(pContext, pFeliCaUserConnection->hDriverConnection,
                           static_PFeliCaWriteDataCompleted, pFeliCaUserConnection,
                           pFeliCaUserConnection->pReaderToCardBuffer, nOffset,
                           pFeliCaUserConnection->pCardToReaderBuffer,
                           NAL_MESSAGE_MAX_LENGTH );
}
            ...
            W_ERROR nError)
{
   ...
   pFeliCaUserConnection->nError = nError;
   PDFCPostContext3(&pFeliCaUserConnection->sCallbackContext, P_DFC_TYPE_FELICA, nDataLength, nError);
}

/* See Client API Specifications */
void PFeliCaExchangeData(
      tContext* pContext,
      W_HANDLE hUserConnection,
      tPBasicGenericDataCallbackFunction * pCallback,
      void * pCallbackParameter,
      ...
      W_HANDLE * phOperation)
{
   ...
   {
      PDebugError("PFeliCaExchangeData: W_ERROR_BAD_PARAMETER");
   ...
   if ( pFeliCaUserConnection->nError == P_ERROR_STILL_PENDING )
   {
      PDebugError("PFeliCaExchangeData: W_ERROR_BAD_STATE");
   ...
   if(phOperation != null)
   {
      *phOperation = PBasicCreateOperation( pContext, null, null );
      ...
      {
         PDebugError("PFeliCaExchangeData: PBasicCreateOperation : out of resource");
			      ...
			      const uint8_t	nIndexSubSystem)
{
   ...
   if ( nError != W_SUCCESS )
   {
      PDebugError("WFeliCaCreateSubSystemConnection: INVALID HANDLE");
   ...
   {
      PDebugError("WFeliCaCreateSubSystemConnection: Sub system number ( %d ) > ( %d ) MAX SUB SYSTEM NUMBER",
                  nIndexSubSystem,
                  P_FELICA_MAX_SUB_SYSTEM);
   ...
   {
      PDebugError("WFeliCaCreateSubSystemConnection: A connection for the subsystem[%d] already exists", nIndexSubSystem);
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, W_ERROR_EXCLUSIVE_REJECTED );
      return;
   }


   /* Check if an operation is still pending */
   /* Verify if a Lock is mandatory */
   if ( pFeliCaUserConnection->nError == P_ERROR_STILL_PENDING )
   {
      PDebugError("WFeliCaCreateSubSystemConnection: W_ERROR_BAD_STATE");
   ...
   if ( pFeliCaUserConnection->nProtocol == P_PROTOCOL_STILL_UNKNOWN )
   {
      PDebugError("WFeliCaCreateSubSystemConnection: W_ERROR_CONNECTION_COMPATIBILITY");
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, W_ERROR_CONNECTION_COMPATIBILITY );
      return;
   }
               ...
               const tWFeliCaConnectionInfo *	pFeliCaConnectionInfo)
{
   ...
   if ( nError != W_SUCCESS )
   {
      PDebugError("PFeliCaSelectCard: INVALID HANDLE");
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, nError );
      return;
   }
   ...
   if(pFeliCaUserConnection->nError == P_ERROR_STILL_PENDING)
   {
      PDebugError("PFeliCaSelectCard : BAD STATE");
      PDFCPostContext2( &sCallbackContext, P_DFC_TYPE_FELICA, W_ERROR_BAD_STATE );
      return;
   }
...
static bool static_PFeliCaDriverCheckCardMatchConfiguration(
               tContext* pContext,
  ...
  tFeliCaDriverConnection* pFeliCaDriverConnection;
   ...
   if ( nError == W_SUCCESS )
   {
      ...
      if (  ( pReaderToCardBuffer == null )
         ...
         || ( nReaderToCardBufferLength > P_FELICA_BUFFER_MAX_SIZE - 1 )
