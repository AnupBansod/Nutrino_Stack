...
static const tchar g_aLibraryImplementation[] = OPEN_NFC_PRODUCT_VERSION_S;
static const tchar g_aLibraryVersion40[] = {'4', '.', '0', 0 };
static const tchar g_aLibraryVersion41[] = {'4', '.', '1', 0 };
static const tchar g_aLibraryVersion42[] = {'4', '.', '2', 0 };
static const tchar g_aLibraryVersion[] = OPEN_NFC_PRODUCT_SIMPLE_VERSION_S;


/* See header */
const tchar* PBasicGetLibraryVersion(void)
{
   return g_aLibraryVersion;
}

/* See header */
const tchar* PBasicGetLibraryImplementation(void)
{
         ...
         const tchar* pVersionString )
{
   ...
   if(nError == W_SUCCESS)
   {
      ...
      if(nError == W_SUCCESS)
      {
         ...
         {
            tchar aDriverImplementation[65];
...
/* See header file */
bool PBasicGenericSyncPrepare(
            tPBasicGenericSyncParameters* pParam)
{
   pParam->nResult = W_SUCCESS;
   pParam->res.hHandle = W_NULL_HANDLE;

   if(CSyncCreateWaitObject(&pParam->hWaitObject) == false)
   {
            ...
            void* pObject )
{
   ...
   static_PBasicCancelOperation(pContext, pOperationInfo, true);
         ...
         W_HANDLE hSubOperation)
{
   ...
   if(pOperationInfo->nOperationState == P_OPERATION_STATE_CANCELLED)
   {
      ...
      static_PBasicCancelOperation(pContext, pSubOperationInfo, false);
      PHandleClose(pContext, hSubOperation);
      return W_SUCCESS;
   }
         ...
         W_HANDLE hOperation )
{
   ...
   if (PHandleIsUser(pContext, hOperation) == false)
   {
   ...
   {
      static_PBasicCancelOperation( pContext, pOperationInfo, false );
...
{
   return PHandleGetAllPropertyNumber(pContext, hConnection, pnPropertyNumber, true);  /* Retrieve only visible properties */
}
         ...
         uint32_t nArrayLength )
{
   ...
   nError = PHandleGetAllPropertyNumber(pContext, hConnection, &nPropertyNumber, true);  /* Retrieve only visible properties */
      ...
      && ( nPropertyNumber != 0 ) )
   {
      ...
      {
         nError = PHandleGetAllProperties(pContext, hConnection, pPropertyArray, nArrayLength, true);  /* Retrieve only visible properties */
      }
   }
...
{
   if (true == PReaderUserIsPropertyVisible(nPropertyIdentifier))
   {
...
loop:
   ...
   switch(nError)
   {
      ...
      case W_SUCCESS:
         ...
         {
            nError = (bSomeCall != false)? W_SUCCESS : W_ERROR_NO_EVENT;
         }
         break;
   ...
   if ((nError == W_ERROR_NO_EVENT) && (bWait != false))
   {
      goto loop;
   }
            ...
            tContext * pContext )
{
   ...
   do
   {
      ...
      nError = PDFCDriverPumpEvent(pContext, true);
   }
