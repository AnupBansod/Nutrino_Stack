 ...
 **/
static bool static_PNDEF2GenGetProperties(
            tContext* pContext,
            void* pObject,
 ...
 **/
static bool static_PNDEF2GenCheckProperties(
            tContext* pContext,
            void* pObject,
            uint8_t nPropertyValue )
{
   return  (W_PROP_NFC_TAG_TYPE_2_GENERIC != nPropertyValue) ? false : true;
}
               ...
               tNDEF2GenConnection* pNDEF2GenConnection)
{
   ...
   P14P3UserExchangeData(
      ...
      (pNDEF2GenConnection->hOperation != W_NULL_HANDLE) ? &hOperation : W_NULL_HANDLE,
      false);  /* no answer crc */

   if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
   {
      if(PBasicAddSubOperationAndClose(pContext, pNDEF2GenConnection->hOperation, hOperation) != W_SUCCESS)
      {
         PDebugError("static_PNDEF2GenSelectSectorPart1: error returned by PBasicAddSubOperationAndClose()");
      }
   }
}
               ...
               uint32_t nSector)
{
   ...
   if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
   {
      if(PBasicAddSubOperationAndClose(pContext, pNDEF2GenConnection->hOperation, hOperation) != W_SUCCESS)
      {
         PDebugError("static_PNDEF2GenSelectSectorPart2: error returned by PBasicAddSubOperationAndClose()");
      }
   }
}
               ...
               const uint8_t* pBuffer)
{
   ...
   if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
   {
      if(PBasicAddSubOperationAndClose(pContext, pNDEF2GenConnection->hOperation, hOperation) != W_SUCCESS)
      {
         PDebugError("static_PNDEF2GenWriteRaw: error returned by PBasicAddSubOperationAndClose()");
      }
   }
}
               ...
               uint8_t* pBuffer)
{
   ...
   if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
   {
      if(PBasicAddSubOperationAndClose(pContext, pNDEF2GenConnection->hOperation, hOperation) != W_SUCCESS)
      {
         PDebugError("static_PNDEF2GenReadRaw: error returned by PBasicAddSubOperationAndClose()");
      }
   }
}
      ...
      W_ERROR    nError)
{
   ...
   if((nError == W_SUCCESS) && (pNDEF2GenConnection->hOperation != W_NULL_HANDLE))
   {
      if(PBasicGetOperationState(pContext, pNDEF2GenConnection->hOperation) == P_OPERATION_STATE_CANCELLED)
      {
         PDebugWarning("static_PNDEF2GenSelectSectorReadWriteAutomaton: operation cancelled");
   ...
   if (nError != W_SUCCESS)
   {
      ...
      PDFCPostContext2(&pNDEF2GenConnection->sCacheCallbackContext, P_DFC_TYPE_NDEF, nError);
      return;
   }
   ...
   switch (pNDEF2GenConnection->nCurrentOperationState)
   {
      ...
      case P_NDEF2GEN_STATE_BEGIN_READ_WRITE:
         ...
         if (pNDEF2GenConnection->bWrite != false)
         {
 ...
 *
 * @param[in]  bWrite  true = write. false = read.
 *
            ...
            W_ERROR nError )
{
   ...
   if(nError == W_SUCCESS)
   {
      if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
      {
         if(PBasicGetOperationState(pContext, pNDEF2GenConnection->hOperation)
            == P_OPERATION_STATE_CANCELLED)
         {
            PDebugWarning("static_PNDEF2GenSendResult: operation cancelled");
            nError = W_ERROR_CANCEL;
         }
      }
   }
   ...
   if(pNDEF2GenConnection->hOperation != W_NULL_HANDLE)
   {
      PBasicSetOperationCompleted(pContext, pNDEF2GenConnection->hOperation);
      PHandleClose(pContext, pNDEF2GenConnection->hOperation);
      pNDEF2GenConnection->hOperation = W_NULL_HANDLE;
   }

   CDebugAssert(pNDEF2GenConnection->bOperationPending != false);
   pNDEF2GenConnection->bOperationPending = false;

   /* Send the error */
   PDFCPostContext2(
      &pNDEF2GenConnection->sCallbackContext,
      P_DFC_TYPE_NDEF,
      nError );
            ...
            uint8_t* pnSNLength )
{
   ...
   if (W_SUCCESS != nError)
   {
      PDebugError("PNDEF2GenCheckMifare: could not get pNDEF2GenConnection buffer");
      return nError;
   }

   /* Copy UID */
   CMemoryCopy(pSerialNumber,
               pNDEF2GenConnection->aUID,
               pNDEF2GenConnection->nUIDLength);
   *pnSNLength = pNDEF2GenConnection->nUIDLength;
            ...
            uint8_t nProperty )
{
   ...
   PDFCPostContext2(&sCallbackContext, P_DFC_TYPE_NDEF, W_SUCCESS);
            ...
            W_HANDLE *phOperation )
{
   ...
   if(phOperation != null)
   {
      *phOperation = PBasicCreateOperation( pContext, null, null );
      ...
      {
         PDebugError("PNDEF2GenRead: Cannot create the operation");
         nError = W_ERROR_OUT_OF_RESOURCE;
         goto return_error;
      }
   }
   ...
   if ( pNDEF2GenConnection->bOperationPending != false )
   {
      PDebugError("PNDEF2GenRead: Operation already pending");
   ...
   CDebugAssert(pNDEF2GenConnection->hOperation == W_NULL_HANDLE);
   if(phOperation != null)
   {
      if((nError = PHandleDuplicate(pContext, *phOperation, &pNDEF2GenConnection->hOperation))
            != W_SUCCESS)
      {
         PDebugError("PNDEF2GenRead: error returned by PHandleDuplicate()");
         goto return_error;
      }
   }

   /* Store the connection information */
   pNDEF2GenConnection->bOperationPending = true;
   ...
   /* Increment the reference count to keep the connection object alive during
      the operation. The reference count is decreased in static_PNDEF2GenSendResult()
...
return_error:
   ...
   {
      PBasicSetOperationCompleted(pContext, *phOperation);
   }
            ...
            W_HANDLE *phOperation )
{
   ...
   if(phOperation != null)
   {
      *phOperation = PBasicCreateOperation( pContext, null, null );
      if ( *phOperation == W_NULL_HANDLE )
      {
         PDebugError("PNDEF2GenWrite: Cannot create the operation");
         nError = W_ERROR_OUT_OF_RESOURCE;
         goto return_error;
      }
   }
   ...
   if ( pNDEF2GenConnection->bOperationPending != false )
   {
      PDebugError("PNDEF2GenWrite: Operation already pending");
      nError = W_ERROR_BAD_STATE;
      goto return_error;
   }
   ...
   if(phOperation != null)
   {
      if((nError = PHandleDuplicate(pContext, *phOperation, &pNDEF2GenConnection->hOperation))
            != W_SUCCESS)
      {
         PDebugError("PNDEF2GenWrite: error returned by PHandleDuplicate()");
         goto return_error;
      }
   }
            ...
            uint32_t nSectorOffset )
{
   ...
   if ( pNDEF2GenConnection->bOperationPending != false )
   {
      PDebugError("PNDEF2GenDirectWrite: Operation already pending");
      nError = W_ERROR_BAD_STATE;
      goto return_error;
   }
