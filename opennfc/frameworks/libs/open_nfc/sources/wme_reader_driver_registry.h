 ...
 *
 * @return  true if the card matches the configuration, false otherwise.
 **/
typedef bool tPReaderDriverCheckCardMatchConfiguration(
            tContext* pContext,
            uint32_t nProtocolBF,
            const uint8_t* pDetectionConfigurationBuffer,
            uint32_t nDetectionConfigurationLength,
...
/**
  * Checks if a SE listener is registered in the reader registry and if a connection has been established
  * with the SE
  *
  * @param[in] pContext The context
  *
  * @return true if a connection is established with the SE
  */
