         ...
         uint32_t nResultCode)
{
   ...
   PDFCPost2(pContext, P_DFC_TYPE_NFC_HAL_SERVICE, static_PNALServiceConnectCompleted, pCallbackParameter, nResultCode);
   ...
   if(pContext != null)
   {
      ...
      for(nServiceIdentifier = 0; nServiceIdentifier < NAL_SERVICE_NUMBER; nServiceIdentifier++)
      {
         ...
         while(pOperation != null)
         {
            ...
            static_PNALServiceCancelOperation(pContext, pInstance, pOperation, false);
         ...
         tContext* pContext)
{
   ...
   if ((pInstance->bWritePending == false ) && (pInstance->bInNALCallback == true))
   {
      PDFCPost0(pContext, P_DFC_TYPE_NFC_HAL_SERVICE, static_PNALServiceExecuteNextOperation);
      return;
   }

   if ((pInstance->bWritePending == false ) && (pInstance->bInNALCallback == false))
   {
      for(nServiceIdentifier = 0; nServiceIdentifier < NAL_SERVICE_NUMBER; nServiceIdentifier++)
      {
         if(pInstance->aOperationLockedArray[nServiceIdentifier] == false)
         {
            tNALServiceOperation* pOperation = pInstance->aOperationListHeadArray[nServiceIdentifier];
                  ...
                  uint8_t nServiceIdentifier)
{
   ...
   CDebugAssert(pInstance->aOperationLockedArray[nServiceIdentifier] != false);

   pInstance->aOperationLockedArray[nServiceIdentifier] = false;
}
         ...
         uint32_t nReceptionCounter )
{
   ...
   /* Check write flag */
   CDebugAssert(pInstance->bWritePending == true);
                  ...
                  void* pCallbackParameter )
{
   ...
   pInstance->bWritePending = true;

#ifdef P_TRACE_ACTIVE
#ifdef P_NFC_HAL_TRACE

   PNALFrameLogBuffer(pContext, pInstance->aSendBuffer, nDataBufferLength + 2, false);
                  ...
                  void* pCallbackParameter )
{
   ...
   PNALFrameLogBuffer(pContext, pInstance->aSendBuffer, nDataBufferLength + 3, false);
...
static bool static_PNALServiceRecvEventReadCompleted(
                  tContext* pContext,
                  tNALServiceOperation* pOperation,
                  uint8_t* pBuffer,
                  uint32_t nLength,
                  ...
                  bool bGracefull )
{
   ...
   PDFCPost3(
      pContext, P_DFC_TYPE_NFC_HAL_SERVICE,
      pOperation->op.s_send_event.pCallbackFunction,
      pOperation->op.s_send_event.pCallbackParameter,
      W_ERROR_CANCEL, 0);
}
                  ...
                  bool bGracefull )
{
   static_PNALServiceCheckOperation(pContext, pOperation);
    ...
    PDFCPost4(
      pContext, P_DFC_TYPE_NFC_HAL_SERVICE,
      pOperation->op.s_execute.pCallbackFunction,
      pOperation->op.s_execute.pCallbackParameter,
      0,
      W_ERROR_CANCEL,
      0);
}
...
static bool static_PNALServiceExecuteReadCompleted(
                  tContext* pContext,
                  tNALServiceOperation* pOperation,
                  uint8_t* pBuffer,
                  uint32_t nLength,
                  ...
                  bool bGracefull )
{
   ...
   PDFCPost3(
      pContext, P_DFC_TYPE_NFC_HAL_SERVICE,
      pOperation->op.s_get_parameter.pCallbackFunction,
      pOperation->op.s_get_parameter.pCallbackParameter,
      0,
      W_ERROR_CANCEL);
}
...
static bool static_PNALServiceGetParameterReadCompleted(
                  tContext* pContext,
                  tNALServiceOperation* pOperation,
                  uint8_t* pBuffer,
                  uint32_t nLength,
                  ...
                  bool bGracefull )
{
   ...
   PDFCPost2(
      pContext, P_DFC_TYPE_NFC_HAL_SERVICE,
      pOperation->op.s_set_parameter.pCallbackFunction,
      pOperation->op.s_set_parameter.pCallbackParameter,
      W_ERROR_CANCEL);
}
...
static bool static_PNALServiceSetParameterReadCompleted(
                  tContext* pContext,
                  tNALServiceOperation* pOperation,
                  uint8_t* pBuffer,
                  uint32_t nLength,
         ...
         uint32_t nReceptionCounter)
{
   ...
   PNALFrameLogBuffer(pContext, pInstance->aReceptionBuffer, nLength, true);
         ...
         void* pCallbackContext)
{
   ...
   PContextLock(pContext);
   PDFCPost1(pContext, P_DFC_TYPE_NFC_HAL_SERVICE, static_PNALServiceTimerHandler, null);
   PContextReleaseLock(pContext);
}
         ...
         tNALServiceOperation* pOperation)
{
   ...
   static_PNALServiceCancelOperation(pContext, pInstance, pOperation, true);
}
