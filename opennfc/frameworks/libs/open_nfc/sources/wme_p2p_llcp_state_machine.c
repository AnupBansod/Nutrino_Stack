   ...
   )
{
   ...
   switch (pLLCPInstance->sRcvPDUDescriptor.nType)
   {
      case LLCP_PDU_CONNECT :
         ...
         if (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sMIUX.bIsPresent == true)
         {
         ...
         if (pLLCPInstance->sRcvPDUDescriptor.sPayload.sParams.sRW.bIsPresent == true)
         {
         ...
         {
            pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sMIUX.bIsPresent = true;
         ...
         {
            pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sRW.bIsPresent = true;
            pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sRW.nRW = pConnection->pP2PSocket->sConfig.nLocalRW;
         }

         pLLCPInstance->sXmitPDUDescriptor.sPayload.sParams.sSN.bIsPresent = false;
   ...
   )
{
   ...
   if (pConnection->pConnectPDU != null)
   {
      if (pLLCPInstance->sRcvPDUDescriptor.nType == LLCP_PDU_CONNECT)
      {
         ...
         pConnection->pConnectPDU->bIsDiscarded = true;
   ...
   )
{
   ...
   */
   ...
   if (pConnection->bAtLeastOneIPDUSent == true)
   {
      if (pLLCPInstance->sRcvPDUDescriptor.nNR == pConnection->nVS)
      {
         PDebugError("static_PLLCPCheckNRNS NR(%d) == VS(%d)\n",  pLLCPInstance->sRcvPDUDescriptor.nNR, pConnection->nVS);
         nErrorFlag |= LLCP_FRMR_FLAG_R;
      }
   }
...
#if 0 /* @todo */
   if (pConnection->bAtLeastOneIPDUReceived == true)
   {
      ...
      for (nNR = pLLCPInstance->sRcvPDUDescriptor.nNR ; nNR != pConnection->nVS; IncMod16(nNR))
      {
         ...
         {
            PDebugError("static_PLLCPCheckNRNS N(R) %d becomes equal to V(SA) %d prior V(S) %d\n",  pLLCPInstance->sRcvPDUDescriptor.nNR, pConnection->nVSA, pConnection->nVS);
            nErrorFlag |= LLCP_FRMR_FLAG_R;
            break;
         }
      }
   ...
   if (pConnection->bAtLeastOneIPDUReceived == true)
   {
      if (pLLCPInstance->sRcvPDUDescriptor.nType == LLCP_PDU_I) {
         ...
         {
            PDebugError("static_PLLCPCheckNRNS N(S)%d >= V(RA) %d + RW(L) %d\n",  pLLCPInstance->sRcvPDUDescriptor.nNS, pConnection->nVRA, pConnection->nRWL);
         ...
         {
            PDebugError("static_PLLCPCheckNRNS N(S) %d != V(R) %d\n",  pLLCPInstance->sRcvPDUDescriptor.nNS, pConnection->nVR);
...
)
{
   ...
   switch (pLLCPInstance->sRcvPDUDescriptor.nType)
   {
      case LLCP_PDU_I :       /* data traffic */
         ...
         pConnection->bAtLeastOneIPDUReceived = true;
         ...
         if (PLLCPConnectionStartXmitSDU(pContext, pConnection) == false)
         {
...
)
{
   ...
   switch (pLLCPInstance->sRcvPDUDescriptor.nType)
   {
      ...
      case LLCP_PDU_DISC :   /* DISC collision */

         PDebugTrace("static_PLLCPConnectionStateMachineDisconnecting : received DISC");
