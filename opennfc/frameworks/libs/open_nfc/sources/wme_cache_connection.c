            ...
            tCacheBlock* pCacheBlock)
{
   ...
   return (tCacheBlock*)PUtilConvertUint32ToPointer(ALIGN(PUtilConvertPointerToUint32(pCacheBlock), 4));
}
 ...
 */
   ...
   pCacheConnection->nTotalLengthInUse =
    ((uint8_t*)static_PCacheGetNextBlock(pCacheBlock)) - pCacheMemory;
                   ...
                   uint32_t nIndex )
{
   ...
   pCacheConnection->bIsCacheModified = true;
}
               ...
               tContext* pContext )
{
   ...
   tCacheConnectionInstance* pCacheConnection = PContextGetCacheConnectionInstance(pContext);
   W_ERROR nError;
   if(pCacheConnection->bIsCacheModified != false)
   {
      ...
      {
         pCacheConnection->bIsCacheModified = false;
      }
   }
            ...
            uint32_t nIdentifier)
{
   ...
   while(((uint8_t*)pCacheBlock) < (pCacheMemory + pCacheConnection->nTotalLengthInUse))
   {
      ...
      {
         uint32_t nLengthToCopy;
         CDebugAssert(((uint8_t*)pNextCacheBlock) <= (pCacheMemory + pCacheConnection->nTotalLengthInUse));

         nLengthToCopy = (pCacheMemory + pCacheConnection->nTotalLengthInUse) - ((uint8_t*)pNextCacheBlock);
         if(nLengthToCopy != 0)
         {
            CMemoryMove(pCacheBlock, pNextCacheBlock, nLengthToCopy);
         }

         pCacheConnection->nTotalLengthInUse -= ((uint8_t*)pNextCacheBlock) - ((uint8_t*)pCacheBlock);
      }
   }
}
