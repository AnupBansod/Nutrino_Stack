...
#define P_14443_3_TYPEB_DETECTION_MESSAGE_SIZE_MAX           265
...
static bool static_P14P3UserGetProperties(
            tContext* pContext,
            void* pObject,
            uint8_t* pPropertyArray );

/* Check properties connection callback */
static bool static_P14P3UserCheckProperties(
            tContext* pContext,
            void* pObject,
...
static W_ERROR static_P14P3UserParseTypeB(
            ...
            uint32_t nLength )
{
   uint32_t nLayerResponseIndex;
   uint32_t nLayerResponseLength;
   uint32_t nIndex = 0;
   ...
   /* CID */
   p14P3UserConnection->sPart3ConnectionInfo.sTypeB.bIsCIDSupported = ( ( pBuffer[nIndex + 10] & 0x01 ) == 0x01 ) ? true: false;
   if ( p14P3UserConnection->sPart3ConnectionInfo.sTypeB.bIsCIDSupported != false )
   {
   ...
   /* NAD */
   p14P3UserConnection->sPart3ConnectionInfo.sTypeB.bIsNADSupported = ( ( ( pBuffer[nIndex + 10] >> 1 ) & 0x01 ) == 0x01 ) ? true: false;
   if ( p14P3UserConnection->sPart3ConnectionInfo.sTypeB.bIsNADSupported != false )
   {
   ...
   **/
   ...
   {
      /* higher layer response is carried*/
      nLayerResponseIndex = nIndex + 1;
      nLayerResponseLength = nLength - P_14443_3_TYPEB_DETECTION_MESSAGE_SIZE_MIN;
            ...
            uint32_t nLength )
{
                     ...
                     nLength ) ) != W_SUCCESS )
   {
      ...
      p14P3UserConnection->nProtocol = P_PROTOCOL_STILL_UNKNOWN;
      goto return_function;
   }
            ...
            W_HANDLE hConnection )
{
   ...
   if ( nError == W_SUCCESS )
   {
      if ( p14P3UserConnection->bIsPart4 == false )
      {
...
static void static_P14P3UserExchangeDataCancel(tContext* pContext, void* pCancelParameter, bool bIsClosing);
...
void P14Part3ExchangeData(
            ...
            W_HANDLE* phOperation )
{
   bool bCheckResponseCRC = true;

   if ( (PBasicCheckConnectionProperty(pContext, hConnection, W_PROP_MIFARE_UL) == W_SUCCESS) ||
        (PBasicCheckConnectionProperty(pContext, hConnection, W_PROP_MY_D_MOVE) == W_SUCCESS) ||
        (PBasicCheckConnectionProperty(pContext, hConnection, W_PROP_MY_D_NFC) == W_SUCCESS) )
   {
      ...
      {
         PDebugTrace("P14Part3ExchangeData: No CRC for Mifare UL and MyD WRITE operation");
         bCheckResponseCRC = false;
      }
   }
