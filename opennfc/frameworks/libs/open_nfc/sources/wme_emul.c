         ...
         uint8_t nPropertyIdentifier )
{
   ...
   case W_PROP_NFC_TAG_TYPE_4_B:
      bValue = ((nProtocols & W_NFCC_PROTOCOL_CARD_ISO_14443_4_B) != 0)?true:false;
      break;
   case W_PROP_ISO_14443_4_A:
   case W_PROP_NFC_TAG_TYPE_4_A:
      bValue = ((nProtocols & W_NFCC_PROTOCOL_CARD_ISO_14443_4_A) != 0)?true:false;
      break;
   default:
      return false;
   }
...
static void static_PEmulCloseDriverCompleted(
      ...
      W_ERROR nError)
{
   PDFCPostContext2(pCallbackParam, P_DFC_TYPE_ISO_EMUL, nError);
         ...
         void * pCallbackParameter )
{
   ...
   if (pCallbackContext != null)
   {
      ...
      if (nError != W_SUCCESS)
      {
         PDFCPost2(pContext, P_DFC_TYPE_ISO_EMUL, static_PEmulCloseDriverCompleted, pCallbackContext, nError);
      }
   }
   else
   {
      ...
      PDFCPostContext2(&sCallbackContext, P_DFC_TYPE_ISO_EMUL, W_ERROR_OUT_OF_RESOURCE);
   }
}
...
tHandleType g_sEmulDriverConnection = {  static_PEmulDriverDestroyConnection, null, null, null, null };
            ...
            W_ERROR nError)
{
   ...
   {
      if (pEmulCardInfo->bFromUser == true)
      {
         PDFCDriverPostCC2(
            ...
            tEmulCardInfo* pEmulCardInfo )
{
   ...
   || (pEmulCardInfo->nState == P_EMUL_DRIVER_SENDING_DATA))
   {
      ...
      pEmulCardInfo->bDestructionScheduled = true;
      return;
   }
   ...
   pEmulCardInfo->bReaderDataAvailable = false;
   ...
   switch ( nEventIdentifier )
   {
      case NAL_EVT_CARD_SEND_DATA:
         ...
         if(pEmulCardInfo->nCardState != P_EMUL_CARD_STATE_SELECTED)
         {
            ...
            if(pEmulCardInfo->bIsEventCallbackSet != false)
            {
               if (pEmulCardInfo->bFromUser == true)
               {
                  PDFCDriverPostCC2(
         ...
         {
            if(pEmulCardInfo->bReaderDataAvailable != false)
            {
            ...
            {
               pEmulCardInfo->bReaderDataAvailable = true;
            }
            ...
            uint32_t nReceptionCounter )
{
   ...
   if(pEmulCardInfo->nState == P_EMUL_DRIVER_SENDING_DATA)
   {
      ...
      if(pEmulCardInfo->bDestructionScheduled != false)
      {
         PDebugTrace("static_PEmulDriverSendEventCompleted: destruction scheduled");
         pEmulCardInfo->bDestructionScheduled = false;
         static_PEmulDriverInitiateDestruction( pContext, pEmulCardInfo );
         return;
      }
   }
   ...
   {
      PDebugError(
         "static_PEmulDriverSendEventCompleted: nError = %s", PUtilTraceError(nError) );
   }
   else
   {
            ...
            W_ERROR nError)
{
   ...
   if ( nError == W_SUCCESS )
   {
      ...
      {
         PDebugWarning("static_PEmulDriverRegisterCompleted: bad state");
         nError = W_ERROR_BAD_STATE;
      }
   }
   else
   {
      PDebugError( "static_PEmulDriverRegisterCompleted: received %s",
         PUtilTraceError(nError) );
   }

   if(nError != W_SUCCESS)
   {
      PDebugError( "static_PEmulDriverRegisterCompleted: sending %s",
         PUtilTraceError(nError) );
            ...
            void* pEventCallbackParameter )
{
   ...
   if ( ( nError == W_SUCCESS ) && ( pEmulCardInfo != null ) )
   {
      ...
      if ((pEmulCardInfo->nState == P_EMUL_DRIVER_OPEN_PENDING)
       || (pEmulCardInfo->nState == P_EMUL_DRIVER_ACTIVE))
      {
         pEmulCardInfo->bIsEventCallbackSet = true;
   ...
   if ( nError == W_SUCCESS )
   {
      if(pEmulCardInfo != null)
      {
         ...
         && (pEmulCardInfo->nState != P_EMUL_DRIVER_CLOSE_PENDING))
         {
            ...
            pEmulCardInfo->bIsCloseCallbackSet = true;
            ...
            bool bFromUser)
{
   ...
   case W_NFCC_PROTOCOL_CARD_ISO_14443_4_A:
      ...
      if ( pEmulConnectionInfo->sCardInfo.s14A.bSetCIDSupport != false )
      {
   ...
   case W_NFCC_PROTOCOL_CARD_P2P_TARGET :
      ...
      pEmulCardInfo->aDataBuffer[nSetParameterLength++] = pEmulConnectionInfo->sCardInfo.sP2PTarget.nTimeOutRTX;
                  ...
                  uint32_t* pnActualDataLength)
{
   ...
   nError = PHandleGetObject(pContext, hDriverHandle, P_HANDLE_TYPE_EMUL_DRIVER_CONNECTION, (void**)&pEmulCardInfo);
   if ( ( nError == W_SUCCESS ) && ( pEmulCardInfo != null ) )
   {
      if(pEmulCardInfo->nState != P_EMUL_DRIVER_ACTIVE)
      {
            ...
            uint32_t nDataLength )
{
   ...
   nError = PHandleGetObject(pContext, hDriverConnection, P_HANDLE_TYPE_EMUL_DRIVER_CONNECTION, (void**)&pEmulCardInfo);
   if ( ( nError == W_SUCCESS ) && ( pEmulCardInfo != null ) )
   {
      if(pEmulCardInfo->nState != P_EMUL_DRIVER_ACTIVE)
      {
