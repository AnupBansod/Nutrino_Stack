         ...
         tDFCElement* pDFC)
{
   if((pDFC->nFlags & P_DFC_FLAG_EXTERNAL) != 0)
   {
      ...
      PContextSetIsInCallbackFlag(pContext, true);
#endif /* P_CONFIG_USER */
...
#if (P_BUILD_CONFIG == P_CONFIG_USER)
      PContextSetIsInCallbackFlag(pContext, false);
#endif /* P_CONFIG_USER */
   }
   else
   {
...
void PDFCPostInternal(
            ...
            void* pParam1 )
{
   tDFCElement* pDFC = static_PDFCPostInternal(pContext);

   CDebugAssert( pFunction != null );
   ...
   if(pDFC != null)
   {
      ...
      PDebugTrace("Posting %08X callback-%p( %p )", pDFC->nFlags, pDFC->pFunction,
               pDFC->pParam1 );
   }
}
         ...
         tDFCCallbackContext* pCallbackContext )
{
   ...
   {
      pCallbackContext->bIsExternal = false;
   ...
   else
   {
      ...
      {
         pCallbackContext->bIsExternal = true;
            ...
            uint32_t nParam3)
{
   ...
   {
      if(pCallbackContext->bIsExternal != false)
      {
         ...
         uint32_t nType )
{
   ...
   if((pUserBuffer != null) && (nBufferSize != 0))
   {
            ...
            tContext * pContext )
{
   ...
   if(pUserInstance->bStopLoop == false)
   {
      pUserInstance->bStopLoop = true;
            ...
            void* pParams )
{
   ...
   while((pDriverCC = pUserInstance->pDFCDriverCCQueueFirst) == null)
   {
      ...
      pUserInstance->bIsEventPumpWaitingForSemaphore = true;

      PContextReleaseLock(pContext);
      bInterrupted = CSyncWaitSemaphore(&pUserInstance->hSemaphore);
      PContextLock(pContext);

      pUserInstance->bIsEventPumpWaitingForSemaphore = false;
      ...
      if(pUserInstance->bEventPumpShouldDestroyUserInstance != false)
      {
...
void PDFCDriverInternalFlushCall(
            ...
            tDFCDriverCC* pDriverCC)
{
   if (PContextCheckDriverDFC(pContext, pDriverCC) == true)
   {
      if(pDriverCC->sDFC.nFlags != 0)
      {
            ...
            uint32_t nParam3)
{
   ...
   tDFCDriverCC* pOtherDriverCC;
   bool          bIsInDFCQueue = false;

   if ((PContextCheckDriverDFC(pContext, pDriverCC) == false) || (pDriverCC->sDFC.pFunction == null))
   {
      return;
   }
   ...
   if((nFlags & P_DFC_FLAG_EVENT_HANDLER) != 0)
   {
      if((nFlags & P_DFC_FLAG_EVENT_HANDLER_USED) != 0)
      {
         ...
         CDebugAssert(bIsInDFCQueue == true);
      }
      else
      {
          CDebugAssert(pDriverCC->pNextQueue == null);
          CDebugAssert(bIsInDFCQueue == false);
      }
         ...
         uint32_t nArgsSize)
{
   ...
   if(pDFCQueue->nElementNumber != 0)
   {
      ...
      pArgs[0] = (uint32_t)pDFC->pFunction;
      pArgs[1] = (uint32_t)pDFC->pParam2;
