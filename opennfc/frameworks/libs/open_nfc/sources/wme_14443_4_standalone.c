...
#define P_14443_4_TYPEB_DETECTION_MESSAGE_SIZE_MAX           265        /* ATQB(11) + ATTRIB(254) */

#define P_14443_4_APDU_MAX_SIZE                             261     /* From MR documentation */

/* default configuration parameters*/
#define P_14443_A_4_READER_CONFIG_PARAMETERS                 {0x00, 0x00, 0x00, 0x00, 0x08}
            ...
            W_ERROR nError)
{
   if(p14P4DriverConnection->bFromUser)
   {
      ...
      PDFCDriverPostCC3(
         ...
         P_DFC_TYPE_ISO_14443_4, nlength, nError );
   }
   else
   {
      PDFCPostContext3(
            ...
            bool     bFromUser)
{
   ...
   if ( nError == W_SUCCESS )
   {
      ...
      if(PReaderDriverIsSEListenerActive(pContext) && bFromUser)
      {
         nError = PSEFilterApdu(
            pContext,
            pReaderToCardBuffer, nReaderToCardBufferLength );

         if (nError != W_SUCCESS)
         {
            PDebugError("P14P4DriverExchangeData : PSEFilterApdu() detected %s", PUtilTraceError(nError));
            goto error;
         }
      }
      ...
      CMemoryCopy(
         &p14P4DriverConnection->aReaderToCardBufferNAL[1],
          pReaderToCardBuffer,
   ...
   else
   {
      ...
      if(bFromUser)
      {
         ...
         PDFCDriverPostCC3(
            pDriverCC,
            P_DFC_TYPE_ISO_14443_4, 0, nError );
      }
      else
      {
         tDFCCallbackContext   sCallbackContext;
            ...
            uint32_t nReceptionCounter)
{
   ...
   if(PReaderDriverIsSEListenerActive(pContext) && p14P4DriverConnection->bFromUser)
   {

      W_ERROR nError2 = PSENotifyResponseApdu(
            pContext,
            p14P4DriverConnection->pCardToReaderBuffer,
            nLength,
            nError );

      if (nError2 != W_SUCCESS)
      {
         PDebugError("static_P14P4DriverExecuteCommandCompleted : PSENotifyResponseApdu() error detected %s", PUtilTraceError(nError2));
...
/* Get properties connection callback */
static bool static_P14P4UserGetProperties(
            tContext* pContext,
            void* pObject,
            uint8_t* pPropertyArray );

/* Check properties connection callback */
static bool static_P14P4UserCheckProperties(
            tContext* pContext,
            void* pObject,
   ...
   if (pBuffer[0] > 1 )
   {
      ...
      if ( pBuffer[1] & 0x10 )
      {
         ...
         if ( pBuffer[nOffset] & 0x80 )
         {
            ...
            p14P4Connection->s14typeA4.bIsPPSSupported = false;
         }
         else
         {
            /* We need top manage the PPS mode */
            p14P4Connection->s14typeA4.bIsPPSSupported = true;
         }
      ...
      if ( p14P4Connection->s14typeA4.bIsPPSSupported != false )
      {
      ...
      if ( pBuffer[1] & 0x40 )
      {
         ...
         /* CID */
         p14P4Connection->s14typeA4.bIsCIDSupported = ( ( pBuffer[nOffset] & 0x02 ) == 0x02 ) ? true: false;
         /* Set the default CID to 0 */
         p14P4Connection->s14typeA4.nCID = 0x00;
         /* NAD */
         p14P4Connection->s14typeA4.bIsNADSupported = ( ( pBuffer[nOffset] & 0x01 ) == 0x01 ) ? true: false;
      }
      else
      {
         p14P4Connection->s14typeA4.bIsCIDSupported = false;
         p14P4Connection->s14typeA4.nCID = 0x00;
         p14P4Connection->s14typeA4.bIsNADSupported = false;
      }
      if ( p14P4Connection->s14typeA4.bIsCIDSupported != false )
      {
      ...
      if ( p14P4Connection->s14typeA4.bIsNADSupported != false )
      {
            ...
            uint8_t* pnType )
{
   ...
   if (p14P4UserConnection->sPart3ConnectionInfo.sTypeA.aUID[0] != 0x04)
   {
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
 ...
 * @param[in]  nLength  The length of the response buffer.
 **/
#define P_14443_3_TYPEB_DETECTION_MESSAGE_SIZE_MIN  12
            ...
            uint32_t nLength )
{
   ...
   /* CID */
   p14P4UserConnection->sPart3ConnectionInfo.sTypeB.bIsCIDSupported = ( ( pBuffer[nIndex + 10] & 0x01 ) == 0x01 ) ? true: false;
   if ( p14P4UserConnection->sPart3ConnectionInfo.sTypeB.bIsCIDSupported != false )
   {
   ...
   /* NAD */
   p14P4UserConnection->sPart3ConnectionInfo.sTypeB.bIsNADSupported = ( ( ( pBuffer[nIndex + 10] >> 1 ) & 0x01 ) == 0x01 ) ? true: false;
   if ( p14P4UserConnection->sPart3ConnectionInfo.sTypeB.bIsNADSupported != false )
   {
   ...
   if (P14P4DriverSetTimeout(
           ...
           p14P4UserConnection->nTimeout ) != W_SUCCESS)
   {
      PDebugError("static_P14P3UserParseTypeB : P14P4DriverSetTimeout failed");
   }
            ...
            uint32_t nLength )
{
                     ...
                     nLength ) ) != W_SUCCESS )
   {
      ...
      p14P4UserConnection->nProtocol = P_PROTOCOL_STILL_UNKNOWN;
      goto return_function;
   }
...
static void static_P14P4UserExchangeDataCancel(tContext* pContext, void* pCancelParameter, bool bIsClosing);
            ...
            W_HANDLE* phOperation )
{
   ...
   PDebugTrace("P14Part4ExchangeData");
   ...
   nError = PReaderUserGetConnectionObject(pContext, hConnection,
      P_HANDLE_TYPE_14443_4_USER_CONNECTION, (void**)&p14P4UserConnection);
   if ( nError != W_SUCCESS )
   {
      PDebugError("P14Part4ExchangeData: could not get p14P3UserConnection buffer");
      goto return_error;
   }
   ...
   {
      PDebugError("P14Part4ExchangeData: operation already pending");
   ...
   {
      nError = W_ERROR_CONNECTION_COMPATIBILITY;
      PDebugWarning("P14Part4ExchangeData: unknow protocol 0x%02X", p14P4UserConnection->nProtocol );
      goto return_error;
   }
        ...
        ((pReaderToCardBuffer == null) && (nReaderToCardBufferLength != 0)))
   {
      ...
      PDebugError("P14Part4ExchangeData: pReaderToCardBuffer and nReaderToCardBuffer are not consistent");
      goto return_error;
   }
        ...
        ((pCardToReaderBuffer != null) && (nCardToReaderBufferMaxLength == 0)))
   {
      ...
      PDebugError("P14Part4ExchangeData: pCardToReaderBuffer and nCardToReaderBufferMaxLength are not consistent");
      goto return_error;
   }

   if ( nReaderToCardBufferLength > P_14443_4_APDU_MAX_SIZE )
   {
      PDebugError("P14Part4ExchangeData: W_ERROR_BUFFER_TOO_LARGE for the card input buffer");
   ...
   {
      PDebugError("P14Part4ExchangeData: Cannot allocate the operation");
      goto return_error;
   }
   ...
   if(phOperation != null)
   {
      ...
      if(nError != W_SUCCESS)
      {
         PDebugError("P14Part4ExchangeData: Error returned by PHandleDuplicate()");
   ...
   if (nError != W_SUCCESS)
   {
      PDebugError("P14Part4ExchangeData: Error returned by PContextGetLastIoctlError()");
      PHandleClose(pContext, p14P4UserConnection->hCurrentOperation);
      p14P4UserConnection->hCurrentOperation = W_NULL_HANDLE;
            ...
            uint32_t* pnCardToReaderBufferActualLength )
{
   ...
   PDebugTrace("W14Part4ExchangeDataSync");
            ...
            uint8_t nNAD )
{
   ...
   if ( nError == W_SUCCESS )
   {
      ...
      {
         case W_PROP_ISO_14443_4_A:
            if (p14P4UserConnection->s14typeA4.bIsNADSupported != false)
            {
