   ...
   void * pUserInstance)
{
   ...
   for (;;)
   {
      ...
      if (pInstance->bRequestStop == true)
	   {
...
/* see porting guide*/
bool CUserWaitForServerEvent(void * pUserInstance, bool * bWaitInServer)
{
   tClientInstance * pInstance = (tClientInstance *) pUserInstance;
	char buffer[1];
   ...
   * bWaitInServer = false;
	...
	if (recv(pInstance->nWBasicPumpEventSocket, buffer, 1, 0) != 1)
   {
      PDebugError("CUserWaitForServerEvent : WaitForSingleObject failed %d", errno);
   }
