                                    ...
                                    )
{
   if(nNALError != NAL_RES_OK)
   {
      ...
      pBindingContext->bIsFirmwareUpdateStarted  = false;
   }
         ...
         uint8_t         nStatusCode)
{
   ...
   case P_NFC_HAL_BINDING_UPDATE_EVENT_COMMAND_EXECUTED:
      ...
      {
         uint8_t nError = NAL_RES_BAD_STATE;
         if ( (pBindingContext->nUpdateState & P_NFC_HAL_BINDING_UPDATE_STATUS_PENDING_TIMEOUT) != 0x00 )
         {
            nError = NAL_RES_TIMEOUT;
         }

         static_PNALBindingUpdateSendResult(pBindingContext, pBindingContext->pOperation, nError);
         return;
      }
   ...
   uint8_t    nNALError)
{
   ...
   if (pBindingContext->bInPoll == false)
   {
      PNALDFCPost2(pBindingContext, P_DFC_TYPE_NFC_HAL_BINDING, static_PNALBindingConfigurationCompleted, nError, (uint32_t) nNALError);
   }
   else
   {
      ...
      uint8_t         nStatusCode)
{
   ...
   switch (pBindingContext->nUpdateState)
   {
      ...
      case P_NFC_HAL_BINDING_LOADER_STATE_RECONNECTED :
         ...
         pBindingContext->bInLoader = pBindingContext->bInLoader == true ? false : true;
