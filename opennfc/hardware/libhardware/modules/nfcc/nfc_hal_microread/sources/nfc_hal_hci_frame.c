...
static bool static_PHCIFrameIsStateConnected( tHCIFrameInstance* );
         ...
         tNALBindingContext* pBindingContext)
{
   ...
   for (;;)
   {
      ...
      bEnd = ( pHCIFrame->aRxPacket[ 0 ] & HCI_FRAME_CHAINING_MSK ) ? true : false;
         ...
         uint32_t nSHDLCFrameReceptionCounter)
{
   ...
   if ( static_PHCIFrameIsStateConnected( PNALContextGetHCIFrameInstance( pBindingContext ) ) != false )
   {
         ...
         void*                       pReconnectIndicationCallbackParameter)
{
   ...
   CNALDebugAssert( ( static_PHCIFrameIsStateConnected( pHCIFrame) == false ));
 ...
 **/
static bool static_PHCIFrameIsStateConnected(
         tHCIFrameInstance* pHCIFrame)
{
   return ( pHCIFrame->nState == HCI_FRAME_STATE_CONNECTED ) ? true : false;
}
