...
static bool static_PSHDLCWriteReceptionCtrlFrame(
         tNALBindingContext* pBindingContext,
         uint8_t nCtrlFrame,
         uint32_t nSHDLCFrameReceptionCounter);
         ...
         uint8_t nWindowSize )
{
   ...
   pSHDLCStack->bIsWriteOtherSideReady = true;
}
         ...
         tSHDLCInstance* pSHDLCStack )
{
   ...
   /* Read state-machine */
   pSHDLCStack->bIsReadBufferFull = false;

   /* Write state-machine */
   pSHDLCStack->bIsWriteFramePending = false;
   pSHDLCStack->bIsWriteBufferFull = false;
   pSHDLCStack->bIsRRFrameAlreadySent = false;
}
 ...
 *
 * @return  true if the buffer contains a valid RST frame, false otherwise.
 **/
static bool static_PSHDLCConnectCheckRSTFrame(
         tSHDLCInstance* pSHDLCStack,
         uint8_t* pBuffer,
         uint32_t nLength)
{
...
/* See header file */
NFC_HAL_INTERNAL bool PSHDLCWrite(
         tNALBindingContext* pBindingContext,
         ...
         uint32_t nFrameBufferLength )
{
   ...
   if( nNextToSendFrameId - nLastAckSentFrameId - 1 >= pSHDLCStack->nWindowSize)
   {
      ...
      pSHDLCStack->bIsWriteBufferFull = true;
      return false;
   }
   ...
   *pBuffer++ = (uint8_t)(0x80 | ((nNextToSendFrameId & 0x07) << 3));
   ...
   static_PSHDLCWriteTransmit(pBindingContext, pSHDLCStack, false);
 ...
 **/
static P_NAL_INLINE bool static_PSHDLCReadCheckFreeSize(
         tSHDLCInstance* pSHDLCStack,
         uint32_t nLength)
{
   /* Adding one byte for the length and 4 byte for the counter */
   return ((nLength + 1 + 4) <= (P_SHDLC_READ_BUFFER_SIZE - pSHDLCStack->nReadBufferDataLength))?true:false;
}
      ...
      *pnSHDLCFrameReceptionCounter = nSHDLCFrameReceptionCounter;
      ...
      if( pSHDLCStack->bIsReadBufferFull )
      {
            ...
            >= P_SHDLC_READ_BUFFER_FREE_TRESHOLD)
         {
            ...
            static_PSHDLCWriteTransmit(pBindingContext, pSHDLCStack, true);
         }
      }
   }
...
static P_NAL_INLINE bool static_PSHDLCReadWriteInBuffer(
         tSHDLCInstance* pSHDLCStack,
         uint8_t* pFrameBuffer,
         ...
         uint32_t nSHDLCFrameReceptionCounter)
{
   ...
   if((nCtrlHeader & 0xC0) != 0x80)
   {
      ...
      if(static_PSHDLCConnectCheckRSTFrame(pSHDLCStack, pBuffer, nLength))
      {
         ...
         pSHDLCStack->nNextCtrlFrameToSend = READ_CTRL_FRAME_UA;
    ...
    */
   if (static_PSHDLCWriteReceptionCtrlFrame(pBindingContext, nCtrlHeader, nSHDLCFrameReceptionCounter) != true)
   {
   ...
   if(nLength != 0)
   {
      ...
      if(static_PSHDLCReadCheckFreeSize(pSHDLCStack, nLength) == false)
      {
         ...
         pSHDLCStack->bIsReadBufferFull = true;
   ...
   if(nLength != 0)
   {
      ...
      if(pSHDLCStack->bIsRRFrameAlreadySent == true)
      {
         pSHDLCStack->bIsRRFrameAlreadySent = false;
         ...
         bool bForceAck)
{
   ...
   if(pSHDLCStack->bIsWriteFramePending != false)
   {
   ...
   if((nNextWindowFrameId != nNextToSendFrameId) && (pSHDLCStack->bIsWriteOtherSideReady != false))
   {
   ...
   if(bSendIframe == false)
   {
      ...
      pSHDLCStack->bIsRRFrameAlreadySent = true;
      ...
      PNALMultiTimerSet( pBindingContext,
           ...
           null );
      nLengthToSend = 1;
      if(nNextCtrlFrameToSend != READ_CTRL_FRAME_UA)
      {
   ...
   else if(pSHDLCStack->bIsRRFrameAlreadySent == true)
   {
   ...
   pSHDLCStack->bIsWriteFramePending = true;
   pSHDLCStack->nNextCtrlFrameToSend = READ_CTRL_FRAME_NONE;
...
static uint32_t static_PSHDLCWriteRebuild32BitIdentifier(
         ...
         uint8_t n3bitValue )
{
   uint32_t n32bitValue = pSHDLCStack->nWriteLastAckSentFrameId;
 ...
 *
 * @return       true if the frame must be processed.
 **/
static bool static_PSHDLCWriteReceptionCtrlFrame(
         tNALBindingContext* pBindingContext,
         uint8_t nCtrlFrame,
         uint32_t nSHDLCFrameReceptionCounter)
{
   ...
   if(nMaskedValue == FRAME_RNR_MASK)
   {
      pSHDLCStack->bIsWriteOtherSideReady = false;
   }
   else
   {
      pSHDLCStack->bIsWriteOtherSideReady = true;
         ...
         nPosition++)
      {
               ...
               static_PSHDLCWriteModulo(nPosition, pSHDLCStack->nWindowSize)].aData[1]) & 0x80) != 0)
         {
            ...
            PNALDFCPost2( pBindingContext, P_DFC_TYPE_SHDLC,
               ...
               (void*)nSHDLCFrameReceptionCounter);
         }
      }
