         ...
         uint32_t nPosition)
{
   ...
   pTraceContext->bTraceChainedFrame = (((*pBuffer) & 0x80) != 0)?false:true;

   pBuffer++;
   nLength--;

   if(bTraceChainedFrame != false)
   {
   ...
   else
   {
      ...
      {
          if (pTraceContext->bFromNFCC != true)
          {
            switch(nValue)
            {
   ...
   if((bOptionTrace == false) || (nLength != 0))
   {
