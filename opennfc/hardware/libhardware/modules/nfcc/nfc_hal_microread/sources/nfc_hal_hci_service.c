...
#define P_HCI_SERVICE_MAX_RETRY 3
...
static const uint16_t g_aServicePipe[P_HCI_SERVICE_MAX_ID + 1] =
{
   ...
   0x19 | 0x8000,   /* 0x21   Mode Reader NFC A          ( NFC Device <-> NFC Controller )  */
   0x1A | 0x8000,   /* 0x22   Mode Reader NFC F          ( NFC Device <-> NFC Controller )  */
};
#endif
/*******************************************************************************
   Instance States
         ...
         tHCIServiceOperation* pOperation)
{
   ...
   switch (pBindingContext->nHCIMode)
   {
      ...
      case W_NFCC_MODE_SWITCH_TO_ACTIVE :
         ...
         {
            if (pOperation->bIsSpecialModeCommand == true)
               break;
         }
         break;
   ...
   if ((pOperation != null) && (pInstance->bWaitingForAnswer == false))
   {
      static_PHCIServiceCheckOperation(pOperation);
                  ...
                  tHCIServiceOperation* pOperation )
{
   ...
   pInstance->bWaitingForAnswer = false;
}
                  ...
                  tHCIServiceOperation* pOperation )
{
   ...
   pInstance->bWaitingForAnswer = true;
}
...
static void static_PHCIServiceResetStandByTimer(tNALBindingContext* pBindingContext){
   if(pBindingContext->nAutoStandbyTimeout != 0)
   {
      ...
      if (pBindingContext->bIsLowPowerRequested != false)
      {
   ...
   if (pBindingContext->bRawMode == false)
   {
      ...
      if(nOffset == 0)
      {
         ...
         else if((pBuffer[0] & ETSI_MSG_TYPE_MASK) == ETSI_MSG_TYPE_CMD)
         {
            ...
            if((nHCIMessageReceptionCounter != 0)&&(bAutomaticAnswer != false))
            {
         ...
         bool  bIsSpecialModeCommand)
{
   ...
   CNALDebugAssert(((pInputBuffer != null) || (nInputBufferLength == 0))
   && ((pInputBuffer == null) || (nInputBufferLength != 0)));
   CNALDebugAssert(((pOutputBuffer != null) || (nOutputBufferLength == 0))
...
static void static_PHCIServiceExecuteMultiServiceCommandCompleted(
         ...
         uint32_t nHCIMessageReceptionCounter)
{
   tHCIServiceOperation* pOperation = pCallbackParameter;
         ...
         tHCIServiceOperation* pOperation)
{
   ...
   pOperation->sMultiService.pOutputBuffer[pOperation->sMultiService.nOutputBufferIndex++] = nService;
   ...
   PHCIServiceExecuteCommand(pBindingContext, nService, pOperation,
      ...
      nLength,
      pOperation->sMultiService.pOutputBuffer + pOperation->sMultiService.nOutputBufferIndex + 1,
      pOperation->sMultiService.nOutputBufferLength - pOperation->sMultiService.nOutputBufferIndex - 1 ,
         ...
         tNALBindingContext * pBindingContext)
{
   ...
   while ((psItem = pInstance->pReceivedFrameListHead) != null)
   {
      ...
      static_PHCIServiceFrameReadDataReceived(
            psItem->pBindingContext, psItem->pCallbackParameter, psItem->nOffset, psItem->pBuffer, psItem->nLength, psItem->nHCIMessageReceptionCounter);
...
static bool static_PHCIServiceSendManagementEvent(
   tNALBindingContext       * pBindingContext,
...
static void static_PHCIServiceInactivityTimeoutElapsed(
      ...
      void     * pCallbackParameter)
{
   if (static_PHCIServiceSendManagementEvent(pBindingContext, g_aHCIInactivityEvent, sizeof(g_aHCIInactivityEvent)) == false)
   {
...
static void static_PHCIServiceFrameReconnectIndication(
         ...
         void*     pCallbackParameter )
{
   if (static_PHCIServiceSendManagementEvent(pBindingContext, g_aHCIWakeUpEvent, sizeof(g_aHCIWakeUpEvent)) == false)
   {
