          ...
          uint32_t nReceptionCounter)
{
   ...
   if(nState == P_NFC_HAL_BINDING_BOOT_GET_HDS_OWNER_READER)
   {
      ...
      /* Update the Reader Protocol capabilities value: p2p initiator is carried in 2bytes, process it manually */
      for(nIndex = 0; nIndex < (HCI2_PAR_MGT_HDS_OWNER_READER_MSG_SIZE - 2); nIndex++)
      {
         pBindingContext->nHCIReaderProtocolOpened |= (static_PNALBindingGetHCIReaderProtocolFromPipeID(pBuffer[2 + nIndex]));
      }
      /* Process the P2P initiator case */
      if((pBuffer[nIndex + 2] == 0x20)||
         (pBuffer[nIndex + 3] == 0x20))
      {
         pBindingContext->nHCIReaderProtocolOpened |= HCI2_PROTOCOL_READER_P2P_INITIATOR;
      }
         ...
         uint32_t nReceptionCounter)
{
   ...
   if ((CNALMemoryCompare(pBuffer + 2 , allZeros, 8 ) == 0) || (CNALMemoryCompare(pBuffer + 2, allOnes, 8) == 0))
   {
      ...
      pBindingContext->bInitialBoot = true;
   }
         ...
         uint32_t nHCIMessageReceptionCounter)
{
   ...
   bool bIsReaderLocked = false;
   bool bIsCardLocked = false;
   ...
   /* set lock bits */
   bIsCardLocked = (pBuffer[4] == HCI2_VAL_UNLOCKED)?false:true;
   bIsReaderLocked = (pBuffer[7] == HCI2_VAL_UNLOCKED)?false:true;
                            ...
                            uint8_t nState)
{
   ...
   switch ( nState )
   {
      case P_NFC_HAL_BINDING_BOOT_GET_FIRMWARE_VERSION:
         ...
         pBindingContext->bInitialBoot = false;

         /* we are not in loader */
         pBindingContext->bInLoader = false;
      ...
      case P_NFC_HAL_BINDING_BOOT_OPEN_AND_CLOSE_PIPE:
         ...
         if (pBindingContext->bRegisterDone == false)
         {
            PHCIServiceRegister(
                 pBindingContext,
                 P_HCI_SERVICE_MREAD,
      ...
      case P_NFC_HAL_BINDING_BOOT_CONFIGURE_NFCC:
         ...
         if (pBindingContext->bInitialBoot == true)
         {
      ...
      case  P_NFC_HAL_BINDING_BOOT_GET_PERSISTENT_POLICY:
         ...
         /* Next step will be P_NFC_HAL_BINDING_BOOT_FINISHED */
         break;
      ...
      case P_NFC_HAL_BINDING_BOOT_FINISHED:
         ...
         if(pBindingContext->nNALBindingStatus == P_NFC_HAL_BINDING_INIT_STATUS)
         {
            ...
            pBindingContext->bRegisterDone = true;
         ...
         else
         {
            ...
            if (pBindingContext->bInLoader == false)
            {
      ...
      case P_NFC_HAL_BINDING_BOOT_LOADER_PENDING:
         ...
         /* we are in loader */
         pBindingContext->bInLoader = true;
 ...
 *
 * @return true if the serial number is in the range, inclusive. false otherwise.
 **/
static bool static_PNALBintingCheckSerialNumber(
          ...
          uint32_t nReceptionCounter)
{
   ...
   if(pNALParHardwareInfo->nSEType != P_NFC_HAL_SE_NONE)
   {
      if((static_PNALBintingCheckSerialNumber(pNALParHardwareInfo->aHardwareSerialNumber,
            g_aNoSE_SerialNumberRangeInf1, g_aNoSE_SerialNumberRangeSup1) != false)
      || (static_PNALBintingCheckSerialNumber(pNALParHardwareInfo->aHardwareSerialNumber,
            g_aNoSE_SerialNumberRangeInf2, g_aNoSE_SerialNumberRangeSup2) != false))
      {
   ...
   {
      if((static_PNALBintingCheckSerialNumber(pNALParHardwareInfo->aHardwareSerialNumber,
            g_aJupiterSE_SerialNumberRangeInf1, g_aJupiterSE_SerialNumberRangeSup1) != false)
      || (static_PNALBintingCheckSerialNumber(pNALParHardwareInfo->aHardwareSerialNumber,
            g_aJupiterSE_SerialNumberRangeInf2, g_aJupiterSE_SerialNumberRangeSup2) != false))
      {
