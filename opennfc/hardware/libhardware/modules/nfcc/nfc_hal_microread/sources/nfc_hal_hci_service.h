...
#else
#define P_HCI_SERVICE_MAX_ID            0x21
#endif
...
typedef struct __tHCIServiceOperation
{
   ...
   bool     bIsSpecialModeCommand;        /**< If true, this operation contains an operation dealing with boot, entering / exiting from standby mode */
 ...
 * @param[in]  bIsSpecialModeCommand  If true, the command is a command related to boot,
 *             switching from active to low power, and switching from low power to active
 *             state transitions. If false, the command is only processed when in active state.
 *
 ...
 *
 * @param[in]  bAutomaticAnswer  if true: send automacally the answer to NFCC.
 *                                  false: the application (receiver) will send the answer to NFCC
 *
 * @param[in]  pOperation  The operation structure to use for this operation.
 *
 * @param[in]  pCallbackFunction  The callback function.
